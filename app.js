...
function refreshRecipesList() {
  ...
  if (list.length === 0) {
    container.innerHTML = `
      <div class="empty-state-card" style="text-align: center; padding: 45px 20px; background: var(--bg-card); border-radius: var(--border-radius-md); border: 1.5px dashed var(--border-color); display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; margin-top: 15px;">
        <i class="fas fa-mortar-pestle" style="font-size: 40px; color: var(--primary-green); opacity: 0.65;"></i>
        <div style="font-weight: 700; font-size: 16px; color: var(--text-main);">No Recipes Found</div>
        <p style="font-size: 12.5px; color: var(--text-secondary); max-width: 260px; margin: 0 auto 12px auto; line-height: 1.4;">Add custom culinary recipes or use the AI Meal Suggestions tool to generate ideas from your active inventory.</p>
        <div style="display: flex; gap: 8px;">
          <button class="btn btn-primary" onclick="document.getElementById('btn-meals-create-trigger').click()" style="padding: 8px 16px; font-size: 13px; cursor: pointer; width: auto;"><i class="fas fa-plus" style="margin-right: 6px;"></i> Create Recipe</button>
          <button class="btn btn-outline" onclick="document.getElementById('btn-meals-ideas-trigger').click()" style="padding: 8px 16px; font-size: 13px; cursor: pointer; width: auto;"><i class="fas fa-lightbulb" style="margin-right: 6px;"></i> Get AI Ideas</button>
        </div>
      </div>
    `;
    return;
