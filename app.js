...
function initReceiptScanning() {
  ...
  const startLiveScanningSimulation = () => {
    if (ocrLiveScanInterval) clearInterval(ocrLiveScanInterval);
    if (liveOcrAnimationId) cancelAnimationFrame(liveOcrAnimationId);
    
    const container = document.getElementById("camera-ocr-highlights");
    if (!container) return;
    container.innerHTML = "";

    // 8 mock receipt line elements with specific heights, widths, and detected label details
    const mockLines = [
      { top: 22, left: 15, width: 45, text: "COSTCO WHOLESALE" },
      { top: 30, left: 20, width: 28, text: "DATE: 06/28/2026" },
      { top: 40, left: 15, width: 48, text: "[98%] ORG CHICKEN" },
      { top: 40, left: 68, width: 16, text: "[99%] $18.50" },
      { top: 48, left: 15, width: 40, text: "[96%] WHOLE MILK 1G" },
      { top: 48, left: 68, width: 16, text: "[98%] $4.50" },
      { top: 56, left: 15, width: 34, text: "[94%] AVOCADOS 6PK" },
      { top: 56, left: 68, width: 16, text: "[97%] $6.00" },
      { top: 64, left: 15, width: 44, text: "[92%] ORGANIC SPINACH" },
      { top: 64, left: 68, width: 16, text: "[95%] $5.00" },
      { top: 74, left: 15, width: 24, text: "SUBTOTAL" },
      { top: 74, left: 68, width: 16, text: "$34.00" }
    ];

    const boxElements = mockLines.map(line => {
      const el = document.createElement("div");
      el.style.position = "absolute";
      el.style.border = "1.5px solid var(--primary-green)";
      el.style.background = "rgba(46, 204, 113, 0.06)";
      el.style.borderRadius = "3px";
      el.style.height = "16px";
      el.style.transition = "opacity 0.25s ease, border-color 0.25s ease, background 0.25s ease";
      ...
      label.style.border = "1px solid var(--primary-green)";
      ...
      return { el, label, line, baseTop: line.top, baseLeft: line.left };
    });
    ...
    const animate = () => {
      ...
      boxElements.forEach((box, i) => {
        // Natural hand shaking jitter animation
        const jitterY = Math.sin(tick * 0.04 + i) * 1.3;
        const jitterX = Math.cos(tick * 0.03 + i) * 1.0;
        
        box.el.style.top = `${box.baseTop + jitterY}%`;
        box.el.style.left = `${box.baseLeft + jitterX}%`;
        box.el.style.width = `${box.line.width}%`;

        // Modulate visibility to simulate real-time confidence scanning fluctuations
        const val = Math.sin(tick * 0.015 + i * 1.5);
        if (val > 0.1) {
          box.label.style.opacity = "1";
          box.el.style.border = "1.5px solid var(--primary-green)";
          box.el.style.background = "rgba(46, 204, 113, 0.14)";
          box.el.style.boxShadow = "0 0 6px rgba(46, 204, 113, 0.35)";
          box.el.style.opacity = "1";
        } else if (val > -0.6) {
          box.label.style.opacity = "0";
          box.el.style.border = "1px dashed rgba(46, 204, 113, 0.5)";
          box.el.style.background = "rgba(46, 204, 113, 0.04)";
          box.el.style.boxShadow = "none";
          box.el.style.opacity = "0.85";
        } else {
          // Temporarily out of focus/view
          box.label.style.opacity = "0";
          box.el.style.opacity = "0";
        }
      });
      liveOcrAnimationId = requestAnimationFrame(animate);
    };
    animate();
  };
    ...
    btnUse.addEventListener("click", async () => {
      ...
      if (window.Tesseract && preview.src && preview.src.startsWith("data:")) {
        try {
          ...
          console.log("Real OCR text matched:", recognizedText);
  ...
  function renderOcrHighlights(chosenStore) {
    ...
    const template = OCR_TEMPLATES[chosenStore] || OCR_TEMPLATES["Costco"];
    const items = template.items || [];

    // Generate boxes for each item row
    items.forEach((item, idx) => {
      const topPercent = 20 + idx * 12;
      
      // Item Name Bounding Box
      const nameBox = document.createElement("div");
      nameBox.className = "camera-ocr-highlight-box";
      nameBox.style.left = "15%";
      nameBox.style.top = `${topPercent}%`;
      nameBox.style.width = `${Math.min(50, 20 + item.name.length * 3.5)}%`;
      nameBox.style.height = "6%";
      nameBox.setAttribute("data-label", `TEXT: ${item.name}`);
      container.appendChild(nameBox);

      // Price Bounding Box
      const priceBox = document.createElement("div");
      priceBox.className = "camera-ocr-highlight-box";
      priceBox.style.right = "15%";
      priceBox.style.top = `${topPercent}%`;
      priceBox.style.width = "18%";
      priceBox.style.height = "6%";
      priceBox.setAttribute("data-label", `$${item.price.toFixed(2)}`);
      container.appendChild(priceBox);
    });

    // Total Row Bounding Box
    const totalTop = 20 + items.length * 12 + 5;
    const totalSum = items.reduce((acc, curr) => acc + curr.price, 0);

    const totalNameBox = document.createElement("div");
    totalNameBox.className = "camera-ocr-highlight-box";
    totalNameBox.style.left = "15%";
    totalNameBox.style.top = `${totalTop}%`;
    totalNameBox.style.width = "25%";
    totalNameBox.style.height = "6%";
    totalNameBox.setAttribute("data-label", "TOTAL");
    container.appendChild(totalNameBox);

    const totalValBox = document.createElement("div");
    totalValBox.className = "camera-ocr-highlight-box";
    totalValBox.style.right = "15%";
    totalValBox.style.top = `${totalTop}%`;
    totalValBox.style.width = "20%";
    totalValBox.style.height = "6%";
    totalValBox.setAttribute("data-label", `$${totalSum.toFixed(2)}`);
    container.appendChild(totalValBox);
  }
  ...
  if (list.length === 0) {
    container.innerHTML = `
      <div class="empty-state-card" style="text-align: center; padding: 45px 20px; background: var(--bg-card); border-radius: var(--border-radius-md); border: 1.5px dashed var(--border-color); display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; margin-top: 15px;">
        ...
        <p style="font-size: 12.5px; color: var(--text-secondary); max-width: 260px; margin: 0 auto 12px auto; line-height: 1.4;">Add pantry or freezer items manually, or scan your store receipt using live OCR to load them automatically.</p>
        <div style="display: flex; gap: 8px;">
          <button class="btn btn-primary" onclick="document.getElementById('btn-add-groc-modal').click()" style="padding: 8px 16px; font-size: 13px; cursor: pointer; width: auto;"><i class="fas fa-plus" style="margin-right: 6px;"></i> Add Item</button>
          <button class="btn btn-outline" onclick="document.getElementById('btn-ocr-scan-trigger').click()" style="padding: 8px 16px; font-size: 13px; cursor: pointer; width: auto;"><i class="fas fa-camera" style="margin-right: 6px;"></i> Scan Receipt</button>
        </div>
      </div>
    `;
    return;
    ...
    card.innerHTML = `
      ...
      <div style="display: flex; flex-direction: column; gap: 3px; margin-bottom: 6px; font-size: 9.5px; color: var(--text-secondary); border-left: 2px solid var(--border-color); padding-left: 6px;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 2px;">
          ...
          <span><strong>Cost:</strong> <span style="color: var(--primary-green); font-weight: 700;">$${recipeCost.toFixed(2)}</span></span>
        </div>
      </div>
...
function renderAnalyticsCharts() {
  ...
  if (nutritionContainer) {
    ...
    // Provide realistic fallback counts if user hasn't scheduled/eaten any planner slot yet
    if (eatenCount === 0) {
      actualCalories = 1640;
      actualProtein = 96;
      actualCarbs = 168;
...
function generateMealBuilderIdeas() {
  ...
  container.innerHTML = `<p style='text-align:center; padding:10px; font-size:12px; color:var(--text-secondary);'><i class='fas fa-spinner fa-spin' style='margin-right:6px;'></i>Analyzing current groceries inventory...</p>`;
  
  // Gather active inventory names
  const activeInventory = State.inventory.filter(item => item.currentQty > 0 && item.remainingPercent > 0);
  const activeNames = activeInventory.map(item => item.name);
  
  // Template database of popular meals
  const mealTemplates = [
    {
      name: "Avocado Spinach Omelette",
      category: "Breakfast",
      description: "Fluffy whisked eggs folded with organic baby spinach, sliced avocado, and a pinch of cheese.",
      ingredients: [
        { name: "Organic Eggs", qty: 0.25, unit: "pcs" },
        { name: "Organic Spinach", qty: 0.25, unit: "bag" },
    ...
    },
    {
      ...
      ingredients: [
        { name: "Organic Eggs", qty: 0.25, unit: "pcs" },
        { name: "Wheat Bread", qty: 0.25, unit: "pcs" }
      ],
    ...
    },
    {
      ...
      category: "Lunch",
      description: "Warm chicken breast strips and fresh spinach wrapped in a soft tortilla.",
      ingredients: [
        { name: "Organic Chicken Breast", qty: 0.25, unit: "pcs" },
        { name: "Organic Spinach", qty: 0.25, unit: "bag" }
      ],
  ...
  // Match items from active inventory
  const matches = [];
  mealTemplates.forEach(t => {
    const matchingIngs = [];
    const missingIngs = [];
    t.ingredients.forEach(req => {
      const match = activeNames.find(invName => isIngredientMatch(invName, req.name));
      if (match) {
        matchingIngs.push(match);
      } else {
    ...
    catMatches.forEach(m => {
      ...
      card.innerHTML = `
        ...
        <div style="display: flex; gap: 6px; margin-top: auto; padding-top: 6px; border-top: 1px dashed var(--border-color);">
          <button class="btn btn-primary" onclick="saveAiIdeaDirect('${recipeDataStr}')" style="font-size: 11px; padding: 4px 8px; height: auto; flex: 1;">Save Recipe</button>
