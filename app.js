...
function refreshRecipesList() {
  ...
  list.forEach(recipe => {
    ...
    card.onclick = () => {
      const drop = document.getElementById(`dropdown-${recipe.id}`);
      if (drop) drop.classList.toggle("expanded");
    };

    card.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 4px; margin-bottom: 2px;">
        <span class="recipe-card-title" style="font-size: 13px; font-weight: 700; line-height: 1.2; color: var(--text-main);">${recipe.name}</span>
      </div>
      
      <p class="recipe-card-desc" style="font-size: 10px; margin-bottom: 6px; color: var(--text-secondary); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.25;">${recipe.description}</p>
      
      <div style="display: flex; flex-direction: column; gap: 3px; margin-bottom: 6px; font-size: 9.5px; color: var(--text-secondary); border-left: 2px solid var(--border-color); padding-left: 6px;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-top: auto; padding-top: 4px; border-top: 1px dashed var(--border-color);">
        <span class="recipe-ingredients-check ${matchClass}" style="font-size: 9px; font-weight: 700; padding: 2px 4px; display: inline-flex; align-items: center; gap: 2px;"><i class="fas ${check.available ? 'fa-circle-check' : 'fa-circle-question'}" style="font-size:8px;"></i> ${matchText}</span>
        <div style="display: flex; gap: 2px; align-items: center;">
          <i class="fas fa-pen recipe-edit-btn" style="color: var(--primary-green); font-size: 11px; cursor: pointer; padding: 4px;" onclick="event.stopPropagation(); openRecipeEditModal('${recipe.id}')"></i>
          <i class="fas fa-trash recipe-delete-btn" style="color: var(--accent-orange); font-size: 11px; cursor: pointer; padding: 4px;" onclick="event.stopPropagation(); deleteRecipe('${recipe.id}')"></i>
        </div>
      </div>
      
      <div class="recipe-details-dropdown" id="dropdown-${recipe.id}" style="font-size: 9.5px;">
        <strong>Ingredients:</strong>
        <ul style="padding-left: 14px; margin: 0; font-size: 9px;">
        ...
        <strong style="margin-top: 4px; font-size: 9px; display: block;">Instructions:</strong>
        <p style="margin: 0; font-size: 9px; line-height: 1.2;">${recipe.instructions}</p>
      </div>
    `;
