...
function initOnboardingFlow() {
  ...
  const executeSocialLogin = async (email, name, avatar, keepSigned, password = "oauth-google-login") => {
    try {
      ...
      try {
        ...
        if (email === "jane.doe@gmail.com") {
          ...
          State.user.email = "jane.doe@gmail.com";
          State.user.avatar = "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150";
        } else {
          State.user = {
            name: name || "New Chef",
            email: email,
            ...
            location: "Philadelphia, PA",
            age: 29,
            avatar: avatar || 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23a0aec0"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>',
            goals: ["goal_budget", "goal_healthy", "goal_planning"],
            budgetAmount: 300,
            budgetPeriod: "monthly",
            desiredStorageWeeks: 2,
            healthyEatingFoods: [],
          ...
          State.inventory = [
            { id: "inv_fresh_1", name: "Organic Spinach", category: "Produce", originalQty: 1, currentQty: 1, unit: "bag", price: 3.50, store: "Whole Foods", purchaseDate: new Date().toISOString().split("T")[0], expiryDate: new Date(Date.now() + 5*86400000).toISOString().split("T")[0], storageLoc: "Fridge", timesUsed: 0, costPerUnit: 3.50, notes: "Wash before eating" }
          ];
          State.trips = [];
          State.notifications = [
            { id: "notif_fresh_1", text: "Welcome to Grocerly! Tap the AI Hub to get smart kitchen advice.", urgent: false, read: false }
          ];
          State.wasteLogs = [];
          State.calendar = {};
        }
  ...
  const openOAuthPopup = (provider) => {
    ...
    const url = provider === "Google" ? "google-login.html" : "apple-login.html";
    window.open(url, `${provider}Login`, `width=${width},height=${height},left=${left},top=${top},scrollbars=yes,resizable=yes`);
  };

  document.getElementById("btn-login-google").addEventListener("click", () => openOAuthPopup("Google"));
  document.getElementById("btn-login-apple").addEventListener("click", () => openOAuthPopup("Apple"));
  
  // Listen for message from OAuth popups
  window.addEventListener("message", (event) => {
    if (event.data && event.data.type === "oauth-success") {
      const keepSigned = document.getElementById("login-keep-signed-in").checked;
      executeSocialLogin(event.data.email, event.data.name, event.data.avatar, keepSigned, event.data.password);
    }
  });
...
function refreshDashboard() {
  ...
  if (budgetProgEl) budgetProgEl.innerText = `Spend: $${totalSpend.toFixed(2)} of $${State.user.budgetAmount}`;
...
function renderCalendarMealSlot(dateKey, mealType) {
  ...
  if (recipe) {
    return `
      <div class="calendar-meal-slot filled" onclick="openMealSelectorModal('${dateKey}', '${mealType}')">
        <span class="calendar-meal-slot-type">${mealType}</span>
        <span class="calendar-meal-slot-name">${recipe.name}</span>
        <div class="calendar-meal-slot-clear-btn" onclick="event.stopPropagation(); clearCalendarSlot('${dateKey}', '${mealType}')">
...
function refreshInventoryList() {
  ...
  list.forEach(item => {
    ...
    card.innerHTML = `
      ...
      <div class="inv-card-details-row" onclick="${clickAction}">
...
function refreshRecipesList() {
  ...
  list.forEach(recipe => {
    ...
    // AI-driven Image Integration based on recipe name
    let searchTerm = recipe.name.split(' ')[0].toLowerCase();
    let imgUrl = `https://loremflickr.com/150/150/${encodeURIComponent(searchTerm)},meal/all`;
    if (recipe.name.includes("Toast")) imgUrl = "https://images.unsplash.com/photo-1525351484163-7529414344d8?w=150";
    else if (recipe.name.includes("Bowl")) imgUrl = "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=150";
    else if (recipe.name.includes("Pasta")) imgUrl = "https://images.unsplash.com/photo-1546548970-71785318a17b?w=150";
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnGrocTrigger && modalGroc) {
    btnGrocTrigger.addEventListener('click', () => modalGroc.classList.add('active'));
  }
