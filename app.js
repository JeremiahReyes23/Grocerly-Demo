...
function recalculateInventoryRemaining() {
  ...
  if (State.calendar) {
    Object.keys(State.calendar).forEach(dateKey => {
      ...
      if (dayMeals) {
        if (dayMeals.Breakfast) scheduledMeals.push({ date: dateKey, recipeId: dayMeals.Breakfast });
        if (dayMeals.Lunch) scheduledMeals.push({ date: dateKey, recipeId: dayMeals.Lunch });
        if (dayMeals.Dinner) scheduledMeals.push({ date: dateKey, recipeId: dayMeals.Dinner });
      }
    });
  }
...
function renderCalendarMealSlot(dateKey, mealType) {
  ...
  if (recipe) {
    return `
      <div class="calendar-meal-slot filled" style="flex-direction: column; justify-content: flex-start; text-align: center; height: auto; min-height: 54px; padding: 6px 18px 6px 6px; white-space: normal;">
    ...
    return `
      <div class="calendar-meal-slot" onclick="openMealSelectorModal('${dateKey}', '${mealType}')" style="min-height: 54px; white-space: normal;">
...
function initReceiptScanning() {
  ...
  const startLiveScanningSimulation = () => {
    ...
    ocrLiveScanInterval = setInterval(() => {
      const box = document.createElement("div");
      box.className = "ocr-live-glow-box";
      
      const randomWidth = 25 + Math.random() * 30;
      const randomLeft = 15 + Math.random() * 15;
      const randomTop = 15 + Math.random() * 55;
      const randomHeight = 4 + Math.random() * 4;
...
function renderReceiptReview(receiptData) {
  ...
  receiptData.items.forEach((item, index) => {
    ...
    row.innerHTML = `
      ...
      <span style="font-size:11px; margin-left: auto; color: var(--text-secondary); flex-shrink: 0;">$</span>
