...
window.openEditGroceryModal = function(id) {
  ...
  document.getElementById('edit-groc-qty').value = item.currentQty;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnEditSave) {
    btnEditSave.addEventListener('click', () => {
      ...
      const qty = parseFloat(document.getElementById('edit-groc-qty').value) || 1;
      item.currentQty = qty;
      ...
      const catSelect = document.getElementById('edit-groc-cat');
  ...
  if (btnDoneCalc) {
    btnDoneCalc.addEventListener('click', () => {
      ...
      // Calculate total for items bought at this store TODAY
      let total = 0;
      State.inventory.forEach(item => {
        if (item.store === store && item.purchaseDate === today) {
          total += item.price;
        }
      });
