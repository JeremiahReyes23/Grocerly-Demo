...
function initOnboardingFlow() {
  ...
  // Load sample avatars
  const avatarList = document.getElementById("avatar-options-list");
  const avatars = [
  ...
  avatars.forEach((src, idx) => {
    ...
    img.src = src;
    img.className = "avatar-option-item" + (idx === 2 ? " selected" : "");
    img.addEventListener("click", () => {
      document.querySelectorAll(".avatar-option-item").forEach(i => i.classList.remove("selected"));
      img.classList.add("selected");
      State.user.avatar = src;
      const mainAvatar = document.getElementById("profile-avatar-select");
      mainAvatar.innerHTML = `<img src="${src}" alt="Profile avatar">`;
    });
    avatarList.appendChild(img);
  });

  // Initial trigger
  document.getElementById("profile-avatar-select").innerHTML = `<img src="${State.user.avatar}" alt="Profile avatar">`;
...
function refreshDashboard() {
  ...
  document.getElementById("stat-budget-rem").innerText = `$${remainingBudget.toFixed(2)}`;
  ...
  document.getElementById("stat-savings-val").innerText = `$${monthlySavings.toFixed(2)}`;

  // 3. Grocery Lifespan
  document.getElementById("stat-duration-val").innerText = `${State.user.desiredStorageWeeks * 7} days`;
  ...
  document.getElementById("stat-waste-val").innerText = `$${totalWastedVal.toFixed(2)}`;
...
function initReceiptScanning() {
  ...
  // OCR Simulator Click
  const scannerBox = document.getElementById("ocr-scan-trigger");
  scannerBox.addEventListener("click", () => {
    if (scannerBox.classList.contains("scanning")) return;
  ...
  document.getElementById("btn-manual-save").addEventListener("click", () => {
    ...
    const store = document.getElementById("man-store").value || "Generic Store";
  ...
  list.forEach(recipe => {
    ...
    card.innerHTML = `
      <div class="recipe-card-header">
        ...
        <span class="recipe-ingredients-check" id="rec-check-${recipe.id}">Checking...</span>
      </div>
      <p class="recipe-card-desc">${recipe.description}</p>
      
      <div class="recipe-nutri-summary">
        <span>Kcal: <span class="recipe-nutri-val">${recipe.nutrition.calories}</span></span>
    ...
    // Check ingredient quantities available
    updateRecipeCardAvailability(recipe);
  });
}
...
function renderAnalyticsCharts() {
  ...
  document.getElementById("stat-anal-total-spend").textContent = `$${textSpendVal.toFixed(2)}`;
...
function exportForecastedList() {
  ...
  let text = `🛒 GROCERLY SHOPPING LIST\nPrepared on: ${todayStr}\n\n`;
  
  aiCachedIntelligence.shoppingForecast.forEach((item, idx) => {
    text += `${idx + 1}. ${item.name} (${item.qtyNeeded} ${item.unit})\n`;
    text += `   • Category: ${item.category}\n`;
    text += `   • Reason: ${item.reason}\n\n`;
  });
  
  text += `Generated by Grocerly - Buy Smarter. Waste Less.`;
  
  navigator.clipboard.writeText(text).then(() => {
    alert("Shopping list copied to clipboard! You can now paste and share it with your family and friends.");
  }).catch(err => {
