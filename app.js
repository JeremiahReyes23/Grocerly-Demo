...
function initOnboardingFlow() {
  ...
  const executeSocialLogin = async (email, name, avatar, keepSigned) => {
  ...
  window.addEventListener("message", (event) => {
    if (event.data && event.data.type === "oauth-success") {
      const keepSigned = document.getElementById("login-keep-signed-in").checked;
      executeSocialLogin(event.data.email, event.data.name, event.data.avatar, keepSigned);
    }
  });
...
function generateMealIdea() {
  if (!State.inventory || State.inventory.length === 0) {
    alert("You need some groceries in your inventory before we can generate a meal idea!");
    return;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnIdeas) {
    btnIdeas.addEventListener('click', () => {
      if(typeof generateMealIdea === 'function') generateMealIdea();
    });
  }
