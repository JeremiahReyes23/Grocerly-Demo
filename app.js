...
window.replenishItem = function(id) {
  ...
  // Pre-fill Add Grocery Modal
  document.getElementById('groc-name').value = item.name;
  document.getElementById('groc-price').value = item.price;
  document.getElementById('groc-date-purchase').value = new Date().toISOString().split('T')[0];
  
  const qtyInput = document.getElementById('groc-qty');
  const unitSelect = document.getElementById('groc-unit');
  if (unitSelect) {
    unitSelect.value = item.unit || 'pcs';
    if (item.unit === 'fraction') {
      qtyInput.type = 'text';
      qtyInput.value = formatQuantityValue(item.originalQty || item.currentQty, item.unit);
      qtyInput.placeholder = 'e.g. 1/3';
    } else {
      qtyInput.type = 'number';
      qtyInput.value = item.originalQty || item.currentQty;
      qtyInput.placeholder = '';
    }
  }
  
  // Set category selection
  document.querySelectorAll('#groc-cat-grid .grid-option').forEach(el => el.classList.remove('selected'));
  const catEl = document.querySelector(`#groc-cat-grid .grid-option[data-cat="${item.category}"]`);
  if (catEl) catEl.classList.add('selected');
  
  // Set location selection
  document.querySelectorAll('#groc-loc-grid .grid-option').forEach(el => el.classList.remove('selected'));
  const locEl = document.querySelector(`#groc-loc-grid .grid-option[data-cat="${item.storageLoc}"]`);
  if (locEl) locEl.classList.add('selected');
  
  document.getElementById('groc-store').value = item.store || 'Costco';
  document.getElementById('groc-notes').value = item.notes || '';
  
  // Calculate new expiration date based on original shelf life offset
  ...
  document.getElementById('groc-date-exp').value = newExp.toISOString().split('T')[0];
...
function deductRecipeIngredientsInternal(recipe) {
  recipe.ingredients.forEach(req => {
    let quantityToDeduct = req.qty;
    
    // Find matching active inventory items, sort by earliest expiry first
...
window.selectMealForCalendar = function(day, mealType, recipeId, element) {
  ...
  if (recipe) {
    deductRecipeIngredientsInternal(recipe);
  }
...
function addIngredientEditorRow(name = "", qty = 1, unit = "pcs") {
  ...
  row.innerHTML = `
    <input type="text" class="form-control ing-row-name" placeholder="Item name" value="${name}" list="inventory-suggestions" style="flex:2;">
    ...
    <select class="form-control ing-row-unit" style="flex:1.2;">
      ...
      <option value="pks" ${unit==='pks'?'selected':''}>pks</option>
      ...
      <option value="fraction" ${unit==='fraction'?'selected':''}>Fraction</option>
    </select>
  ...
  const selectEl = row.querySelector('.ing-row-unit');
  const qtyEl = row.querySelector('.ing-row-qty');
  if (selectEl && qtyEl) {
    const handler = () => {
      if (selectEl.value === 'fraction') {
        qtyEl.type = 'text';
        qtyEl.placeholder = '1/3';
      } else {
        qtyEl.type = 'number';
        qtyEl.placeholder = '';
      }
    ...
    if (unit === 'fraction') {
      qtyEl.type = 'text';
      qtyEl.value = formatQuantityValue(qty, unit);
      qtyEl.placeholder = '1/3';
    } else {
      qtyEl.type = 'number';
      qtyEl.value = qty;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnGrocSave) {
    btnGrocSave.addEventListener('click', () => {
      ...
      const store = document.getElementById('groc-store').value;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnEditSave) {
    btnEditSave.addEventListener('click', () => {
      ...
      item.store = document.getElementById('edit-groc-store').value;
