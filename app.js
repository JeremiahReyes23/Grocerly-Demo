  ...
  scheduledMeals.forEach(meal => {
    ...
    recipe.ingredients.forEach(ing => {
      ...
      const matchingItems = State.inventory
        .filter(item => item.name.toLowerCase().includes(ing.name.toLowerCase()) && new Date(item.purchaseDate) <= new Date(meal.date))
...
function getRecipeServingsAvailable(recipe) {
  ...
  recipe.ingredients.forEach(ing => {
    const matchingItems = State.inventory.filter(item => item.name.toLowerCase().includes(ing.name.toLowerCase()));
