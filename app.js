...
function generateMealBuilderIdeas() {
  ...
  ClientAPI.request(`/api/ai/ideas${param}`)
    ...
    .catch(err => {
      console.warn("Backend ideas search failed, fallback to local engine:", err);
      // Fallback local match logic
      ...
      templates.forEach(t => {
        ...
        t.ingredients.forEach(req => {
          const match = activeNames.find(invName => invName.toLowerCase().includes(req.name.toLowerCase()) || req.name.toLowerCase().includes(invName.toLowerCase()));
          if (match) {
