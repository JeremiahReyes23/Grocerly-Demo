...
const ClientAPI = {
  ...
  async syncBackend() {
    ...
    try {
      ...
      // Sync calendar
      await this.request('/api/calendar', 'PUT', { email: State.user.email, calendar: State.calendar });
      // Sync custom stores
      await this.request('/api/custom-stores', 'PUT', { email: State.user.email, customStores: State.customStores });
    } catch (e) {
...
async function loadStateFromServer() {
  ...
  try {
    ...
    const calendar = await ClientAPI.request(`/api/calendar?email=${encodeURIComponent(email)}`);
    const customStores = await ClientAPI.request(`/api/custom-stores?email=${encodeURIComponent(email)}`);
