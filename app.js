...
function populateStoreDropdowns() {
  ...
  if (cleanItem.includes(cleanIng) || cleanIng.includes(cleanItem)) {
    return true;
  }
  
  const itemWords = cleanItem.replace(/[^a-z0-9\s]/g, "").split(/\s+/).filter(w => w.length > 2);
  const ingWords = cleanIng.replace(/[^a-z0-9\s]/g, "").split(/\s+/).filter(w => w.length > 2);
  
  if (itemWords.length === 0 || ingWords.length === 0) return false;
  return itemWords.some(w => ingWords.includes(w));
}
...
function recalculateInventoryRemaining() {
  ...
  // Deduct matching ingredient percentages (FIFO, date-sensitive)
  scheduledMeals.forEach(meal => {
    const recipe = State.recipes.find(r => r.id === meal.recipeId);
    if (!recipe || !recipe.ingredients) return;

    recipe.ingredients.forEach(ing => {
      // Find matching inventory items purchased on or before the meal date, sorted by purchaseDate (FIFO)
      const matchingItems = State.inventory
        .filter(item => isIngredientMatch(item.name, ing.name) && new Date(item.purchaseDate) <= new Date(meal.date))
  ...
  if (recipe) {
    return `
      <div class="calendar-meal-slot filled" style="flex-direction: column; justify-content: flex-start; text-align: center; height: auto; min-height: 54px; padding: 6px 18px 6px 6px; white-space: normal;">
        ...
        <div class="calendar-meal-slot-clear-btn" onclick="event.stopPropagation(); clearCalendarSlot('${dateKey}', '${mealType}')" style="position: absolute; top: 3px; right: 3px; display: block; font-size: 11px; padding: 2px; cursor: pointer; color: var(--accent-orange); line-height: 1;">
