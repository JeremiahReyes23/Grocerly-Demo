...
function refreshInventoryList() {
  ...
  list.forEach(item => {
    ...
    card.innerHTML = `
      ...
      <div style="padding: 0 16px; margin-top: 4px; margin-bottom: 8px;" onclick="${clickAction}">
        <div class="timeline-progress-track" style="margin-top: 0; background: var(--border-color); height: 6px; border-radius: 3px;">
        ...
        <div style="display: flex; justify-content: space-between; font-size: 10px; color: var(--text-secondary); margin-top: 4px;">
          <span>Purchased: ${item.originalQty || item.currentQty} ${item.unit}</span>
          <span style="font-weight: 700; color: ${getUsageProgressBarColor(item.remainingPercent)};">${item.remainingPercent.toFixed(0)}% remaining</span>
        </div>
      </div>
