...
// ----------------------------------------------------
// 0. PREMIUM NOTIFICATION & CLOUD STATUS HELPER ENGINE
// ----------------------------------------------------
function showToast(message, type = "success") {
  ...
  // Fade out and remove after 3.2 seconds
  setTimeout(() => {
...
function updateAllAvatarElements() {
  const avatar = (State && State.user && State.user.avatar) || 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23a0aec0"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
  
  const ids = ["dash-avatar-img", "profile-card-img", "profile-card-img-sidebar"];
  ids.forEach(id => {
  ...
  if (select) {
    if (avatar.startsWith("data:image/svg+xml")) {
      select.innerHTML = `${avatar.replace("data:image/svg+xml;utf8,", "")}`;
    } else {
      select.innerHTML = `<img src="${avatar}" alt="Profile avatar" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">`;
    }
  }
}

// Intercept browser alert with toast notification service
window.alert = function(message) {
  const msgLower = message.toLowerCase();
  let type = "info";
  if (msgLower.includes("success") || msgLower.includes("saved") || msgLower.includes("congratulations") || msgLower.includes("welcome") || msgLower.includes("added") || msgLower.includes("replenished") || msgLower.includes("updated") || msgLower.includes("congrats")) {
    type = "success";
  } else if (msgLower.includes("error") || msgLower.includes("fail") || msgLower.includes("cannot") || msgLower.includes("invalid") || msgLower.includes("not match") || msgLower.includes("must equal")) {
    type = "error";
  } else if (msgLower.includes("please") || msgLower.includes("select") || msgLower.includes("fill in") || msgLower.includes("empty") || msgLower.includes("are you sure")) {
    type = "warning";
  }
...
window.toggleRecipeDropdown = function(recipeId, btnEl) {
  const drop = document.getElementById(`dropdown-${recipeId}`);
  if (drop) {
    const isExpanded = drop.classList.toggle("expanded");
    btnEl.innerHTML = isExpanded ? 'Ingredients &and;' : 'Ingredients &or;';
  }
};
...
function updateCloudSyncBadge(status) {
  ...
  if (status === "online") {
    ...
    badge.title = "All data is securely synchronized with your Railway cloud database.";
  } else if (status === "offline") {
    if (icon) icon.className = "fas fa-cloud-arrow-down";
    if (text) text.textContent = "Offline Cache";
    badge.title = "Using local offline storage cache. Data will sync back when the database reconnects.";
  } else if (status === "syncing") {
    if (icon) icon.className = "fas fa-sync-alt";
    if (text) text.textContent = "Syncing...";
    badge.title = "Actively synchronizing changes to cloud database...";
  }
}
...
const GOALS_PRESETS = [
  ...
  { id: "goal_waste", title: "Reduce food waste", icon: "fa-recycle" },
...
const OCR_TEMPLATES = {
  "Costco": {
    ...
    items: [
      { name: "CHK BRST", category: "Meat & Poultry", qty: 2, unit: "pcs", price: 16.90 },
      { name: "ORG EGGS", category: "Dairy & Eggs", qty: 1, unit: "pcs", price: 7.20 },
      { name: "BNNS ORG", category: "Produce", qty: 2, unit: "bag", price: 5.50 },
      { name: "UNK ITEM", category: "Produce", qty: 1, unit: "pcs", price: 5.99 }
    ]
  },
  "Walmart": {
    store: "Walmart Supercenter",
    items: [
      { name: "GRND BF", category: "Meat & Poultry", qty: 1, unit: "pcs", price: 11.20 },
      { name: "WHT BRD", category: "Bakery", qty: 2, unit: "pcs", price: 2.60 },
      { name: "BNNS ORG", category: "Produce", qty: 1, unit: "lbs", price: 1.89 },
      { name: "MLK 2%", category: "Dairy & Eggs", qty: 1, unit: "pcs", price: 4.80 }
    ]
  },
  "Trader Joe's": {
    ...
    items: [
      ...
      { name: "JSMS RCE", category: "Pantry", qty: 1, unit: "pcs", price: 5.99 },
      { name: "OLV OIL", category: "Pantry", qty: 1, unit: "pcs", price: 8.99 },
      { name: "UNK ITEM", category: "Produce", qty: 2, unit: "bag", price: 2.29 }
    ]
  },
  "Aldi": {
    store: "ALDI Stores Inc",
    items: [
      { name: "FRESH STRWBRY", category: "Produce", qty: 2, unit: "box", price: 3.18 },
      { name: "IRISH BUTTER", category: "Dairy & Eggs", qty: 1, unit: "pcs", price: 3.49 },
      { name: "CHK TENDERLOINS", category: "Meat & Poultry", qty: 1, unit: "pcs", price: 8.50 },
  ...
  "Target": {
    ...
    items: [
      { name: "APPLES GALA", category: "Produce", qty: 1, unit: "pcs", price: 3.99 },
      { name: "INSTANT OATMEAL", category: "Pantry", qty: 1, unit: "box", price: 3.29 },
      { name: "PENNE PASTA", category: "Pantry", qty: 3, unit: "pcs", price: 1.19 },
      { name: "UNK ITEM", category: "Pantry", qty: 2, unit: "pcs", price: 2.49 }
    ]
  },
  "Whole Foods": {
    store: "Whole Foods Market",
    items: [
      { name: "ORG CHK WINGS", category: "Meat & Poultry", qty: 1, unit: "pcs", price: 12.49 },
      { name: "ORG BLUEBERRIES", category: "Produce", qty: 1, unit: "box", price: 4.99 },
...
function updateInventoryDatalist() {
  ...
  if (State && State.inventory) {
    State.inventory.forEach(item => {
      if (item.currentQty > 0 && item.remainingPercent > 0) {
        names.add(item.name);
      }
    });
  }
...
function isIngredientMatch(itemName, ingName) {
  if (!itemName || !ingName) return false;
  const cleanItem = itemName.toLowerCase().trim();
  const cleanIng = ingName.toLowerCase().trim();
  
  const NOISE_WORDS = new Set([
    "organic", "fresh", "local", "frozen", "natural", "white", "green", "red", "yellow", "blue",
    "bag", "pack", "pcs", "lbs", "oz", "can", "box", "large", "small", "raw", "cooked", "extra",
    "sweet", "whole", "sliced", "diced", "chopped", "pkg", "pks", "pk", "brand"
  ]);

  const getFoodTokens = (str) => {
    return str.replace(/[^a-z0-9\s]/g, "")
      .split(/\s+/)
      .filter(w => w.length > 2 && !NOISE_WORDS.has(w));
  };

  const itemTokens = getFoodTokens(cleanItem);
  const ingTokens = getFoodTokens(cleanIng);

  if (itemTokens.length === 0 || ingTokens.length === 0) {
    return cleanItem.includes(cleanIng) || cleanIng.includes(cleanItem);
  }

  return itemTokens.some(w => ingTokens.includes(w));
}
...
function recalculateInventoryRemaining() {
  ...
  // Collect all scheduled recipes in the Weekly Planner with their dates
    ...
    Object.keys(State.calendar).forEach(dateKey => {
      ...
      if (dayMeals) {
        ["Breakfast", "Lunch", "Dinner"].forEach(mealType => {
          const slot = getCalendarMeal(dayMeals[mealType]);
          if (slot && slot.status === "Eaten") {
            scheduledMeals.push({ date: dateKey, recipeId: slot.recipeId });
          }
        });
      }
    });
  ...
  // Sort scheduled meals chronologically so FIFO deductions follow date order consistently
  ...
  scheduledMeals.forEach(meal => {
    ...
    recipe.ingredients.forEach(ing => {
      // Find matching inventory items sorted by purchaseDate (FIFO)
      const matchingItems = State.inventory
        .filter(item => isIngredientMatch(item.name, ing.name))
...
function getRecipeServingsAvailable(recipe) {
  if (!recipe.ingredients || recipe.ingredients.length === 0) return 0;
  
  // 1. Calculate base servings from current physical stock
  let minServings = Infinity;
  recipe.ingredients.forEach(ing => {
    const matchingItems = State.inventory.filter(item => isIngredientMatch(item.name, ing.name));
    const totalAvailableFraction = matchingItems.reduce((acc, curr) => acc + ((curr.remainingPercent !== undefined ? curr.remainingPercent : 100) / 100), 0);
    const requiredFraction = parseFloat(ing.qty) || 0.5;

    if (requiredFraction <= 0) return;
    const servings = Math.floor(totalAvailableFraction / requiredFraction);
    if (servings < minServings) {
      minServings = servings;
    }
  });
  
  const basePhysicalServings = minServings === Infinity ? 0 : minServings;

  // 2. Count how many times this recipe is scheduled as "Planned" or "Prepared"
  let plannedCount = 0;
  if (State.calendar) {
    Object.keys(State.calendar).forEach(dateKey => {
      const dayMeals = State.calendar[dateKey];
      if (dayMeals) {
        ["Breakfast", "Lunch", "Dinner"].forEach(mealType => {
          const slot = getCalendarMeal(dayMeals[mealType]);
          if (slot && slot.recipeId === recipe.id && (slot.status === "Planned" || slot.status === "Prepared")) {
            plannedCount++;
          }
        });
      }
  ...
  // 3. Return remaining schedulable servings
  return Math.max(0, basePhysicalServings - plannedCount);
}
...
function getRecipeCostValue(recipe) {
  ...
  recipe.ingredients.forEach(ing => {
    ...
    let requiredFraction = parseFloat(ing.qty) || 0.5;

    for (let item of matchingItems) {
      if (requiredFraction <= 0) break;
      const itemFraction = (item.remainingPercent !== undefined ? item.remainingPercent : 100) / 100;
      if (itemFraction > 0) {
        const pull = Math.min(requiredFraction, itemFraction);
        totalCost += pull * (item.price || 0);
        requiredFraction -= pull;
      }
    }
    
    if (requiredFraction > 0 && matchingItems.length > 0) {
      totalCost += requiredFraction * (matchingItems[0].price || 0);
    }
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Custom stores manager add button
  const btnAddNewCustomStore = document.getElementById("btn-add-new-custom-store");
  const newCustomStoreNameInput = document.getElementById("new-custom-store-name");
  if (btnAddNewCustomStore && newCustomStoreNameInput) {
    btnAddNewCustomStore.addEventListener("click", () => {
      const newName = newCustomStoreNameInput.value.trim();
      if (!newName) {
        alert("Please enter a store name.");
        return;
      }
      if (!State.customStores) State.customStores = [];
      if (State.customStores.includes(newName)) {
        alert(`"${newName}" is already in your custom stores!`);
        return;
      }
      State.customStores.push(newName);
      newCustomStoreNameInput.value = "";
      saveStateToStorage();
      populateStoreDropdowns();
  ...
  // Profile Preferred Currency select event
  ...
  if (profileCurrencySelect) {
    profileCurrencySelect.addEventListener("change", (e) => {
      ...
      State.user.currency = e.target.value;
      saveStateToStorage();
      
      // Update all prices instantly
      refreshDashboard();
      refreshInventoryList();
      if (typeof refreshReceiptFoldersList === "function") refreshReceiptFoldersList();
  ...
  // Inline Contact support submit bind
  const btnContactSupport = document.getElementById("btn-contact-support-submit");
  const txtContactMessage = document.getElementById("contact-support-message");
  if (btnContactSupport && txtContactMessage) {
    btnContactSupport.addEventListener("click", () => {
      const msg = txtContactMessage.value.trim();
      if (!msg) {
        alert("Please type a concern or feedback message.");
        return;
      }
      
      ClientAPI.request("/api/feedback", "POST", {
        email: State.user.email || "guest@grocerly.app",
        name: State.user.name || "Guest Chef",
        message: msg
      })
      .then(() => {
        showToast("Your support request has been sent to our inbox! We will get back to you shortly.", "success");
        txtContactMessage.value = "";
        
        // If logged in as admin, refresh the dashboard lists instantly
        if (State.user.isAdmin) {
          refreshAdminDashboard();
        }
      })
      .catch(err => {
        console.error("Failed to send support feedback", err);
...
function transitionToPage(pageId) {
  ...
  const mainTabs = ["page-dashboard", "page-groceries", "page-meals", "page-inventory", "page-analytics", "page-profile", "page-admin"];
...
function initOnboardingFlow() {
  // Login Actions
  const btnSignup = document.getElementById("btn-login-signup");
  if (btnSignup) {
    btnSignup.addEventListener("click", async () => {
      ...
      if (!email || !pass) {
        alert("Please fill in email and password to sign up.");
        return;
      }
      if (!email.includes("@")) {
      ...
      try {
        ...
        State.user.uid = response.googleUid || email;
        ...
        State.customStores = response.customStores || [];
        ...
        saveStateToStorage();
        alert("Account created successfully! Welcome to Grocerly.");
        transitionToPage("page-profile-setup");
        document.getElementById("app-bottom-nav").classList.remove("active");
      } catch (err) {
        console.error("Signup failed:", err);
        alert(err.message || "Failed to create account. Email may already be in use.");
      }
    });
  }
  ...
  document.getElementById("btn-login-signin").addEventListener("click", async () => {
    ...
    try {
      const response = await ClientAPI.request('/api/auth/login', 'POST', { email, password: pass });
       // Apply profile data
      State.user = response.profile;
      State.user.uid = response.googleUid;
      State.inventory = response.inventory;
      State.recipes = response.recipes;
      State.trips = response.trips;
      State.notifications = response.notifications;
  ...
  const handleGoogleResponse = async (response) => {
    try {
      ...
      State.user.uid = apiResponse.googleUid;
      ...
      State.calendar = apiResponse.calendar || {};
      State.customStores = apiResponse.customStores || [];
  ...
  window.onload = async () => {
    let clientId = "721951712629-f4i7m3j2vk4bkana2vjbap26res637g3.apps.googleusercontent.com";
    try {
      const config = await ClientAPI.request('/api/auth/config');
      if (config && config.clientId) {
        clientId = config.clientId;
      }
    } catch (e) {
      console.warn("Failed to fetch backend Google OAuth Client ID, using default:", e);
    }
    ...
    const tryInitGoogle = () => {
      if (window.google && window.google.accounts) {
        ...
        const container = document.getElementById("btn-login-google-container");
        if (container) {
...
function renderDynamicQuestion(index) {
  ...
  nextBtn.onclick = () => {
    ...
    if (currentQuestionIndex < dynamicQuestionsQueue.length - 1) {
      ...
      document.getElementById("congrats-summary").innerText = `Your profile is complete! Based on your goals, we've set a daily macro target of ${State.user.macroGoals.calories} kcal, and a ${State.user.budgetPeriod} grocery budget of ${formatCurrency(State.user.budgetAmount)}.`;
      transitionToPage("page-congrats");
    }
  };
...
function initMainTabs() {
  document.querySelectorAll("#app-bottom-nav .nav-item").forEach(item => {
    item.addEventListener("click", (e) => {
      ...
      if (tab === "admin") refreshAdminDashboard();
    });
  });
}
...
function refreshDashboard() {
  ...
  const labelEditBudget = document.getElementById("lbl-edit-budget-amount");
  if (labelEditBudget) labelEditBudget.textContent = `Budget Amount (${symbol})`;
  const labelAddItemPrice = document.getElementById("lbl-add-item-price");
  if (labelAddItemPrice) labelAddItemPrice.textContent = `Price (${symbol})`;
  const labelEditItemPrice = document.getElementById("lbl-edit-item-price");
  if (labelEditItemPrice) labelEditItemPrice.textContent = `Price (${symbol})`;
  ...
  const isJirar = State.user && State.user.email && State.user.email.toLowerCase() === "jirartixtry@gmail.com";
  const navItemAdmin = document.getElementById("nav-item-admin");
  if (navItemAdmin) {
    navItemAdmin.style.display = (isJirar && State.user.isAdmin) ? "flex" : "none";
  }
  ...
  if (budgetRemEl) budgetRemEl.innerHTML = formatCurrency(remainingBudget);
  if (budgetProgEl) budgetProgEl.innerHTML = `Spent: <span style="color: var(--accent-orange); font-weight: 700;">-${formatCurrency(totalSpend)}</span> of ${formatCurrency(State.user.budgetAmount)}`;
  ...
  if (savingsValEl) savingsValEl.innerHTML = `${formatCurrency(monthlySavings)}${estLabel}`;
  ...
  if (wasteValEl) wasteValEl.innerHTML = `${formatCurrency(totalWastedVal)}${estLabel}`;
...
function renderDashboardGroceryList() {
  ...
  State.inventory.forEach(item => {
    ...
    tr.innerHTML = `
      ...
      <td style="text-align: center; color: var(--text-secondary);">${getStorageLocationBadgeHtml(item.storageLoc)}</td>
...
function renderMealPlanningCalendar() {
  ...
  if (label) {
    if (State.plannerWeekOffset === 0) {
      label.textContent = "This Week";
    } else if (State.plannerWeekOffset === 1) {
      label.textContent = "Next Week";
    } else if (State.plannerWeekOffset === -1) {
      label.textContent = "Last Week";
    } else {
      label.textContent = `${State.plannerWeekOffset > 0 ? '+' : ''}${State.plannerWeekOffset} Weeks`;
    }
  }

  // Bind navigation and reset buttons
  const btnPrev = document.getElementById("btn-planner-prev-week");
  if (btnPrev) {
    btnPrev.onclick = () => {
      State.plannerWeekOffset--;
      renderMealPlanningCalendar();
    };
  }
  const btnNext = document.getElementById("btn-planner-next-week");
  if (btnNext) {
    btnNext.onclick = () => {
      State.plannerWeekOffset++;
      renderMealPlanningCalendar();
  ...
  if (btnReset) {
    btnReset.onclick = () => {
      if (confirm("Are you sure you want to clear all scheduled meals for the currently selected week? This will replenish your grocery items for these dates.")) {
        const todayForReset = new Date();
        todayForReset.setDate(todayForReset.getDate() + (State.plannerWeekOffset * 7));
        const currentDayForReset = todayForReset.getDay();
        const distanceToMondayForReset = currentDayForReset === 0 ? -6 : 1 - currentDayForReset;
        const mondayDateForReset = new Date(todayForReset);
        mondayDateForReset.setDate(todayForReset.getDate() + distanceToMondayForReset);

        for (let i = 0; i < 7; i++) {
          const d = new Date(mondayDateForReset);
          d.setDate(mondayDateForReset.getDate() + i);
          const dateKey = d.toISOString().split("T")[0];
          if (State.calendar && State.calendar[dateKey]) {
  ...
  // Calculate dates shifted by plannerWeekOffset
  const today = new Date();
  today.setDate(today.getDate() + (State.plannerWeekOffset * 7));
...
function renderCalendarMealSlot(dateKey, mealType) {
  ...
  const slot = getCalendarMeal(mealVal);
  const recipe = slot ? State.recipes.find(r => r.id === slot.recipeId) : null;
  ...
  if (recipe) {
    ...
    return `
      <div class="calendar-meal-slot filled" style="flex-direction: column; justify-content: flex-start; text-align: center; height: auto; min-height: 72px; padding: 6px 18px 24px 6px; white-space: normal; position: relative;">
        <div onclick="openMealSelectorModal('${dateKey}', '${mealType}')" style="flex: 1; width: 100%; display: flex; flex-direction: column; overflow: hidden; cursor: pointer; align-items: center;">
          <span class="calendar-meal-slot-type">${mealType}</span>
          <span class="calendar-meal-slot-name" style="white-space: normal; word-break: break-word; line-height: 1.15; font-weight: 700; font-size: 10px; display: block; margin-top: 2px;">${recipe.name}</span>
        </div>
        
        <div style="position: absolute; bottom: 4px; left: 4px; display: flex; align-items: center; gap: 3px;">
          <select onchange="updateMealStatus('${dateKey}', '${mealType}', this.value)" style="font-size: 9px; font-weight: 700; padding: 1px 2px; background: var(--bg-card); color: ${statusColor}; border: 1px solid var(--border-color); border-radius: 4px; cursor: pointer; height: auto; line-height: 1;">
            <option value="Planned" style="color: var(--text-secondary);" ${slot.status === "Planned" ? "selected" : ""}>Planned</option>
            <option value="Prepared" style="color: var(--accent-orange);" ${slot.status === "Prepared" ? "selected" : ""}>Prepared</option>
            <option value="Eaten" style="color: var(--primary-green);" ${slot.status === "Eaten" ? "selected" : ""}>Eaten</option>
            <option value="Skipped" style="color: #ef476f;" ${slot.status === "Skipped" ? "selected" : ""}>Skipped</option>
          </select>
        </div>

        <div class="calendar-meal-slot-clear-btn" onclick="event.stopPropagation(); clearCalendarSlot('${dateKey}', '${mealType}')" style="position: absolute; top: 2px; right: 5px; display: block; font-size: 10px; font-weight: 800; cursor: pointer; color: var(--accent-orange); line-height: 1; font-family: system-ui, -apple-system, sans-serif;">
          X
        </div>
      </div>
    `;
    ...
    return `
      <div class="calendar-meal-slot empty-slot" onclick="openMealSelectorModal('${dateKey}', '${mealType}')" style="min-height: 72px; white-space: normal; border: 1px dashed var(--border-color); border-radius: var(--border-radius-sm); background: rgba(0,0,0,0.01); display: flex; flex-direction: column; align-items: center; justify-content: center; transition: all 0.2s ease; cursor: pointer;">
        <span class="calendar-meal-slot-type" style="font-size: 9px; text-transform: uppercase; font-weight: 700; opacity: 0.5;">${mealType}</span>
        <span class="calendar-meal-slot-name" style="color: var(--primary-green); font-size: 10px; font-weight: 600; display: inline-flex; align-items: center; gap: 4px; margin-top: 2px;"><i class="fas fa-plus" style="font-size: 9px;"></i> Plan</span>
      </div>
    `;
  }
}

window.updateMealStatus = function(dateKey, mealType, newStatus) {
  if (State.calendar && State.calendar[dateKey] && State.calendar[dateKey][mealType]) {
    const mealVal = State.calendar[dateKey][mealType];
    const slot = getCalendarMeal(mealVal);
    slot.status = newStatus;
    State.calendar[dateKey][mealType] = slot;
...
function openMealSelectorModal(day, mealType) {
  ...
  if (State.recipes.length === 0) {
    ...
    State.recipes.forEach(recipe => {
      const servingsAvailable = getRecipeServingsAvailable(recipe);
      const isAvailable = servingsAvailable > 0;
      
      const clickHandler = isAvailable 
        ? `selectMealForCalendar('${day}', '${mealType}', '${recipe.id}', this)`
        : ``;
      
      optionsHTML += `
        <div class="social-account-select-row" style="margin-bottom: 8px; padding: 12px; border-color: ${isAvailable ? 'var(--primary-green)' : 'var(--border-color)'}; opacity: ${isAvailable ? '1' : '0.4'}; cursor: ${isAvailable ? 'pointer' : 'not-allowed'}; pointer-events: ${isAvailable ? 'auto' : 'none'};" onclick="${clickHandler}">
          ...
          <span class="recipe-tag" style="background-color: ${isAvailable ? 'var(--primary-green-light)' : 'var(--border-color)'}; color: ${isAvailable ? 'var(--primary-green)' : 'var(--text-secondary)'}; font-size:9px; border:none; padding:2px 6px;">
            ${isAvailable ? `${servingsAvailable} Left` : 'Out of Stock'}
          </span>
        </div>
      `;
    });
...
window.selectMealForCalendar = function(day, mealType, recipeId, element) {
  ...
  if (servingsAvailable <= 0) {
    alert(`Cannot add "${recipe.name}" to the calendar because there are not enough ingredients remaining in your groceries stock.`);
    return;
  }

  if (!State.calendar[day]) State.calendar[day] = { Breakfast: null, Lunch: null, Dinner: null };
  
  State.calendar[day][mealType] = { recipeId: recipeId, status: "Planned" };
  saveStateToStorage();
  refreshInventoryList();
  refreshDashboard();
...
const OCR_ABBREVIATIONS = {
  "CHK BRST": { name: "Organic Chicken Breast", category: "Meat & Poultry", lowConfidence: false },
  "ORG EGGS": { name: "Organic Eggs 24ct", category: "Dairy & Eggs", lowConfidence: false },
  "BNNS ORG": { name: "Organic Bananas", category: "Produce", lowConfidence: false },
  "GRND BF": { name: "Ground Beef 2lbs", category: "Meat & Poultry", lowConfidence: false },
  "WHT BRD": { name: "Wheat Bread", category: "Bakery", lowConfidence: false },
  "MLK 2%": { name: "2% Milk", category: "Dairy & Eggs", lowConfidence: false },
  "SALMON FILLETS": { name: "Salmon Fillets", category: "Meat & Poultry", lowConfidence: false },
  "JSMS RCE": { name: "Jasmine Rice 5lbs", category: "Pantry", lowConfidence: false },
  "OLV OIL": { name: "Organic Olive Oil", category: "Pantry", lowConfidence: false },
  "FRESH STRWBRY": { name: "Fresh Strawberries", category: "Produce", lowConfidence: false },
  "IRISH BUTTER": { name: "Irish Butter", category: "Dairy & Eggs", lowConfidence: false },
  "CHK TENDERLOINS": { name: "Chicken Tenderloins", category: "Meat & Poultry", lowConfidence: false },
  "FLOUR TORTILLAS": { name: "Flour Tortillas", category: "Bakery", lowConfidence: false },
  "APPLES GALA": { name: "Gala Apples 3lbs", category: "Produce", lowConfidence: false },
  "INSTANT OATMEAL": { name: "Instant Oatmeal", category: "Pantry", lowConfidence: false },
  "PENNE PASTA": { name: "Penne Pasta 1lb", category: "Pantry", lowConfidence: false },
  "ORG CHK WINGS": { name: "Organic Chicken Wings", category: "Meat & Poultry", lowConfidence: false },
  "ORG BLUEBERRIES": { name: "Organic Blueberries", category: "Produce", lowConfidence: false },
  "UNK ITEM": { name: "Unrecognized Item", category: "Produce", lowConfidence: true }
};

function getOcrPreparedImageData(videoOrImg) {
  const tempCanvas = document.createElement("canvas");
  const width = videoOrImg.videoWidth || videoOrImg.naturalWidth || videoOrImg.width || 640;
  const height = videoOrImg.videoHeight || videoOrImg.naturalHeight || videoOrImg.height || 480;
  tempCanvas.width = width;
  tempCanvas.height = height;
  const ctx = tempCanvas.getContext("2d");
  ctx.drawImage(videoOrImg, 0, 0, width, height);
  ...
  try {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      // High-contrast binary thresholding to clean text outlines
    ...
    console.warn("Failed canvas pixel data extraction (CORS or blank context):", e);
  }
  return tempCanvas.toDataURL("image/png");
}

function extractItemsFromOcrText(text) {
  ...
  lines.forEach(line => {
    ...
    // Look for prices, e.g., $12.34 or 12.34
    const priceMatch = cleanLine.match(/(\$?)\s*(\d+[\.,]\d{2})\b/);
    if (priceMatch) {
      const price = parseFloat(priceMatch[2].replace(",", "."));
      let namePart = cleanLine.replace(priceMatch[0], "").trim();
      namePart = namePart.replace(/^[\d\s\*\#\.\-\,\+]+/g, "").replace(/[\d\s\*\#\.\-\,\+]+$/g, "").trim();
      
      if (namePart.length > 2) {
        let category = "Pantry";
        const nameLower = namePart.toLowerCase();
        
        if (nameLower.includes("chk") || nameLower.includes("chicken") || nameLower.includes("beef") || nameLower.includes("meat") || nameLower.includes("pork") || nameLower.includes("fish") || nameLower.includes("salmon") || nameLower.includes("turkey") || nameLower.includes("steak")) {
          category = "Meat & Poultry";
        } else if (nameLower.includes("milk") || nameLower.includes("egg") || nameLower.includes("butter") || nameLower.includes("cheese") || nameLower.includes("yogurt") || nameLower.includes("cream")) {
          category = "Dairy & Eggs";
        } else if (nameLower.includes("bread") || nameLower.includes("toast") || nameLower.includes("tortilla") || nameLower.includes("bun") || nameLower.includes("bagel") || nameLower.includes("bakery")) {
          category = "Bakery";
        } else if (nameLower.includes("banana") || nameLower.includes("apple") || nameLower.includes("avocado") || nameLower.includes("spinach") || nameLower.includes("berry") || nameLower.includes("salad") || nameLower.includes("carrot") || nameLower.includes("produce") || nameLower.includes("lemon") || nameLower.includes("lime") || nameLower.includes("orange") || nameLower.includes("onion") || nameLower.includes("tomato")) {
          category = "Produce";
        }
...
function initReceiptScanning() {
  ...
  let ocrLiveScanInterval = null;
  let liveOcrAnimationId = null;

  const startLiveScanningSimulation = () => {
    const container = document.getElementById("camera-ocr-highlights");
    if (container) container.innerHTML = "";
  };

  const stopLiveScanningSimulation = () => {
    if (liveOcrAnimationId) {
      cancelAnimationFrame(liveOcrAnimationId);
      liveOcrAnimationId = null;
    }
    if (ocrLiveScanInterval) {
      clearInterval(ocrLiveScanInterval);
      ocrLiveScanInterval = null;
    }
    const container = document.getElementById("camera-ocr-highlights");
    if (container) container.innerHTML = "";
  };
  ...
  const customStoreGroup = document.getElementById("ocr-custom-store-group");

  storeChips.forEach(chip => {
    chip.addEventListener("click", () => {
      storeChips.forEach(c => c.classList.remove("selected"));
      chip.classList.add("selected");
      if (chip.dataset.store === "Other") {
        if (customStoreGroup) customStoreGroup.style.display = "block";
      } else {
        if (customStoreGroup) customStoreGroup.style.display = "none";
      }
    });
  });

  // Camera Scanner Binding
  const scanTrigger = document.getElementById("btn-ocr-scan-trigger");
  ...
  const btnUpload = document.getElementById("btn-camera-upload");
  ...
  const openCameraModal = async () => {
    clearOcrHighlights();
    const scanline = document.getElementById("camera-scanline-laser");
    const hudText = document.getElementById("camera-hud-text");
    if (scanline) scanline.style.display = "block";
    if (hudText) {
      hudText.style.display = "block";
      hudText.innerHTML = '<i class="fas fa-expand"></i> Align receipt to detect text automatically';
    }
    try {
  ...
  if (scanTrigger) {
    scanTrigger.addEventListener("click", openCameraModal);
  }

  const stopCameraStream = () => {
    stopLiveScanningSimulation();
    const scanline = document.getElementById("camera-scanline-laser");
    const hudText = document.getElementById("camera-hud-text");
    ...
    fileInput.addEventListener("change", (e) => {
      ...
      if (file) {
        ...
        reader.onload = (event) => {
          ...
          if (hudText) {
            hudText.style.display = "block";
            hudText.innerHTML = '<i class="fas fa-circle-check" style="color: var(--primary-green);"></i> Image Loaded. Ready to process.';
          }
          if (cameraStream) {
            cameraStream.getTracks().forEach(track => track.stop());
            cameraStream = null;
          ...
          renderOcrHighlights(chosenStore);
        };
        reader.readAsDataURL(file);
      }
    });
  ...
  if (btnCapture) {
    btnCapture.addEventListener("click", () => {
      ...
      if (hudText) {
        hudText.style.display = "block";
        hudText.innerHTML = '<i class="fas fa-circle-check" style="color: var(--primary-green);"></i> Image Captured. Ready to process.';
      }
  ...
  if (btnUse) {
    btnUse.addEventListener("click", async () => {
      ...
      overlay.innerHTML = `
        <i class="fas fa-spinner fa-spin" style="font-size: 40px; color: var(--primary-green); margin-bottom: 12px;"></i>
        <div style="font-weight: 700; font-size: 16px;">AI OCR Reading Receipt...</div>
        <div style="font-size: 12px; color: var(--text-secondary); margin-top: 4px;">Translating abbreviations and validating totals</div>
      `;
      document.body.appendChild(overlay);

      // Perform actual OCR if Tesseract library is loaded and preview source is active
      let extractedItems = [];
      if (window.Tesseract && preview.src && preview.src.startsWith("data:")) {
        try {
          const preprocessedDataUrl = getOcrPreparedImageData(preview);
          const result = await Tesseract.recognize(preprocessedDataUrl, 'eng');
          const recognizedText = result.data.text;
          console.log("Real OCR preprocessed text matched:", recognizedText);
          extractedItems = extractItemsFromOcrText(recognizedText);
        } catch (ocrErr) {
          console.warn("Real OCR recognition failed:", ocrErr);
        }
      }
      ...
      setTimeout(async () => {
        ...
        if (extractedItems.length > 0) {
          ...
          try {
            const rawData = await ClientAPI.request('/api/ocr', 'POST', { store: chosenStore });
            ocrTempReceiptData = parseOcrReceiptData(rawData);
            renderReceiptReview(ocrTempReceiptData);
          } catch (err) {
            console.warn("OCR API failed, falling back to local simulation:", err);
            const template = OCR_TEMPLATES[chosenStore];
            ocrTempReceiptData = parseOcrReceiptData(JSON.parse(JSON.stringify(template)));
            renderReceiptReview(ocrTempReceiptData);
          }
        }
      }, 1500);
  ...
  function parseOcrReceiptData(rawData) {
    const parsedItems = rawData.items.map(item => {
      const cleanName = item.name.trim().toUpperCase();
      let translatedName = item.name;
      let category = item.category || "Produce";
      let lowConfidence = false;

      if (OCR_ABBREVIATIONS[cleanName]) {
        translatedName = OCR_ABBREVIATIONS[cleanName].name;
        category = OCR_ABBREVIATIONS[cleanName].category;
        lowConfidence = OCR_ABBREVIATIONS[cleanName].lowConfidence || false;
      } else {
        let matched = false;
        for (let abbr in OCR_ABBREVIATIONS) {
          if (cleanName.includes(abbr) || abbr.includes(cleanName)) {
            translatedName = OCR_ABBREVIATIONS[abbr].name;
            category = OCR_ABBREVIATIONS[abbr].category;
            lowConfidence = OCR_ABBREVIATIONS[abbr].lowConfidence || false;
            matched = true;
            break;
          }
        }
  ...
  function renderOcrHighlights(chosenStore) {
    const container = document.getElementById("camera-ocr-highlights");
    if (container) container.innerHTML = "";
  }

  // Custom Store Creator Chip Binding
  const customStoreInput = document.getElementById("ocr-custom-store-input");
  const addCustomStoreBtn = document.getElementById("btn-ocr-add-custom-store");

  if (addCustomStoreBtn && customStoreInput) {
    addCustomStoreBtn.addEventListener("click", () => {
      const customName = customStoreInput.value.trim();
      if (!customName) {
      ...
      const existingChips = Array.from(document.querySelectorAll("#ocr-store-grid .store-chip"));
      const duplicate = existingChips.some(chip => chip.dataset.store.toLowerCase() === customName.toLowerCase());
      if (duplicate) {
        alert(`"${customName}" is already in the list!`);
        return;
      }

      // Save custom store to persistent State
      if (!State.customStores) State.customStores = [];
      if (!State.customStores.includes(customName)) {
        State.customStores.push(customName);
        saveStateToStorage();
      }

      // Add chip to UI grid
      const otherChip = document.querySelector('#ocr-store-grid .store-chip[data-store="Other"]');
      const newChip = document.createElement("div");
      newChip.className = "store-chip selected";
      newChip.dataset.store = customName;
      newChip.textContent = customName;

      // Register select event on new chip
      newChip.addEventListener("click", () => {
        document.querySelectorAll("#ocr-store-grid .store-chip").forEach(c => c.classList.remove("selected"));
        newChip.classList.add("selected");
        if (customStoreGroup) customStoreGroup.style.display = "none";
      });

      // Insert new chip before Other chip
      otherChip.parentNode.insertBefore(newChip, otherChip);

      // Trigger selection state changes
      document.querySelectorAll("#ocr-store-grid .store-chip").forEach(c => c.classList.remove("selected"));
      newChip.classList.add("selected");

      // Auto create template database entry
      if (!OCR_TEMPLATES[customName]) {
        OCR_TEMPLATES[customName] = {
          store: customName,
          items: [
            { name: "CHK BRST", category: "Meat & Poultry", qty: 2, unit: "pcs", price: 16.90 },
            { name: "MLK 2%", category: "Dairy & Eggs", qty: 1, unit: "pcs", price: 4.20 },
            { name: "UNK ITEM", category: "Produce", qty: 3, unit: "pcs", price: 6.99 }
          ]
        };
      }

      // Clear input and hide group
      customStoreInput.value = "";
      if (customStoreGroup) customStoreGroup.style.display = "none";
      alert(`"${customName}" has been successfully added to your store chips list!`);
    });
  }
  ...
  document.getElementById("btn-ocr-save").addEventListener("click", () => {
    ...
    document.querySelectorAll(".review-item-row").forEach(row => {
      ...
      if (itemData) {
        const nameInput = row.querySelector(".review-name-input");
        const editedName = nameInput ? nameInput.value.trim() : itemData.name;
...
function renderReceiptReview(receiptData) {
  ...
  receiptData.items.forEach((item, index) => {
    ...
    const isLow = item.lowConfidence;
    if (isLow) row.classList.add("low-confidence");
    const warningIcon = isLow ? `<i class="fas fa-triangle-exclamation" style="color: var(--accent-orange); font-size: 13px;" title="Low confidence OCR extraction - please verify"></i>` : '';

    row.innerHTML = `
      ${warningIcon}
      <input type="text" class="form-control review-name-input" value="${item.name}" style="flex: 2; padding: 4px; font-size:12px; height: auto; min-width: 80px;">
      <input type="number" class="form-control review-qty-input" value="${item.qty}" style="width: 50px; padding: 4px; font-size:12px; text-align:center; height: auto; flex-shrink: 0;">
      <span style="font-size:11px; margin-left: 2px; flex-shrink: 0;">${item.unit}</span>
      <span style="font-size:11px; margin-left: auto; color: var(--text-secondary); flex-shrink: 0;" class="currency-symbol-label">${getCurrencySymbol()}</span>
      <input type="number" class="form-control review-price-input" value="${item.price.toFixed(2)}" step="0.01" style="width: 60px; padding: 4px; font-size:12px; text-align:right; height: auto; flex-shrink: 0;">
      <i class="fas fa-trash-can review-item-delete" onclick="deleteReceiptReviewItem(this, ${item.price})" style="flex-shrink: 0;"></i>
    `;
    listContainer.appendChild(row);
  });

  totalLabel.textContent = formatCurrency(receiptSum);
...
function deleteReceiptReviewItem(btnEl, price) {
  ...
  document.getElementById("review-receipt-total").textContent = formatCurrency(totalSum);
}
...
function initInventoryManagement() {
  ...
  // Bind Export Lists Button
  const btnExport = document.getElementById("btn-inventory-export-lists");
  if (btnExport) {
    btnExport.addEventListener("click", () => {
      exportInventoryToShoppingLists();
    });
  }
...
function refreshStoreFiltersUI() {
  const storeChipsContainer = document.getElementById("inventory-store-chips");
  if (!storeChipsContainer) return;
  
  // Find all unique store names in State.inventory
  const uniqueStores = [...new Set(State.inventory.map(item => item.store).filter(Boolean))].sort();
  
  // Save current active store filter to restore it if possible, else default to "All"
  if (activeStoreFilter !== "All" && !uniqueStores.includes(activeStoreFilter)) {
  ...
  allChip.className = `filter-chip${activeStoreFilter === "All" ? " active" : ""}`;
  ...
  allChip.onclick = () => selectStoreFilter("All");
  storeChipsContainer.appendChild(allChip);
  
  // Add chips for other stores
  uniqueStores.forEach(store => {
    ...
    chip.onclick = () => selectStoreFilter(store);
...
function refreshInventoryList() {
  ...
  let list = State.inventory.filter(item => {
    ...
    // Store matching
    const matchesStore = activeStoreFilter === "All" || (item.store && item.store.toLowerCase().trim() === activeStoreFilter.toLowerCase().trim());
    
    return matchesSearch && matchesLoc && matchesCategory && matchesStore;
  });
  ...
  // Calculate and display total price of filtered items
  const totalPrice = list.reduce((sum, item) => sum + (item.price || 0), 0);
  const totalPriceEl = document.getElementById("inventory-total-price");
  if (totalPriceEl) {
    totalPriceEl.textContent = `Total: ${formatCurrency(totalPrice)}`;
  }

  if (list.length === 0) {
    container.innerHTML = `
      <div class="empty-state-card" style="text-align: center; padding: 45px 20px; background: var(--bg-card); border-radius: var(--border-radius-md); border: 1.5px dashed var(--border-color); display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; margin-top: 15px;">
        <i class="fas fa-boxes-packing" style="font-size: 40px; color: var(--primary-green); opacity: 0.65;"></i>
        <div style="font-weight: 700; font-size: 16px; color: var(--text-main);">Your Pantry is Empty</div>
        <p style="font-size: 12.5px; color: var(--text-secondary); max-width: 260px; margin: 0 auto; line-height: 1.4;">Add pantry or freezer items manually, or scan your store receipt using live OCR to load them automatically.</p>
      </div>
    `;
    return;
  }
    ...
    card.innerHTML = `
      ...
      <div class="inv-card-header" onclick="${clickAction}">
        <div class="inv-card-title-group">
          ...
          <span class="inv-card-cat">${item.category} • ${getStorageLocationBadgeHtml(item.storageLoc)}</span>
        </div>
...
function getStorageLocationBadgeHtml(storageLoc) {
  const icon = getStorageIcon(storageLoc);
  if (storageLoc === "Freezer") {
    return `<span style="color: var(--freezer-blue, #0ea5e9); font-weight: 700;"><i class="fas ${icon}" style="color: var(--freezer-blue, #0ea5e9); margin-right: 4px;"></i>FREEZER</span>`;
  }
  return `<span><i class="fas ${icon}" style="margin-right: 4px;"></i>${storageLoc}</span>`;
}

function getCalendarMeal(slot) {
  if (!slot) return null;
  if (typeof slot === 'string') {
    return { recipeId: slot, status: 'Planned' };
  }
  return slot; // already an object
}
...
function formatCurrency(amount) {
  ...
  const current = (State.user && State.user.currency) ? State.user.currency : 'USD';
  const info = currencies[current] || currencies.USD;
...
function getCurrencySymbol() {
  const currencies = {
    USD: '$', CAD: 'C$', EUR: '€', GBP: '£', AUD: 'A$', NZD: 'NZ$', JPY: '¥', INR: '₹', MXN: 'MX$'
  };
  const current = (State.user && State.user.currency) ? State.user.currency : 'USD';
  return currencies[current] || '$';
}

function parseQtyString(val) {
  if (!val) return 0.5;
...
window.openManageStoresModal = function() {
  const modal = document.getElementById("modal-manage-stores");
  if (modal) {
    modal.classList.add("active");
    renderManageStoresList();
  }
...
window.closeManageStoresModal = function() {
  const modal = document.getElementById("modal-manage-stores");
  if (modal) {
    modal.classList.remove("active");
  }
...
window.renderManageStoresList = function() {
  const container = document.getElementById("manage-stores-list");
  const emptyMsg = document.getElementById("manage-stores-empty");
  if (!container) return;
  container.innerHTML = "";
  ...
  customStores.forEach((storeName, index) => {
    const row = document.createElement("div");
    ...
    row.innerHTML = `
      <input type="text" class="form-control" value="${storeName}" onchange="renameCustomStore(${index}, this.value)" style="height: 32px; font-size: 13px; flex: 1; padding: 4px 8px;">
      <button class="btn btn-outline" onclick="deleteCustomStore(${index})" style="height: 32px; width: 32px; padding: 0; border-color: var(--accent-orange); color: var(--accent-orange); display: flex; align-items: center; justify-content: center; flex-shrink: 0;"><i class="fas fa-trash-can" style="margin: 0; font-size: 12px;"></i></button>
    `;
    container.appendChild(row);
  });
};

window.renameCustomStore = function(index, newName) {
  ...
  const oldName = State.customStores[index];
  if (oldName === cleanName) return;
  
  // Update in state
  State.customStores[index] = cleanName;
  
  // Reassign in inventory if any items reference it
  State.inventory.forEach(item => {
    if (item.store === oldName) {
      item.store = cleanName;
    }
  });
...
window.deleteCustomStore = function(index) {
  const storeName = State.customStores[index];
  
  // Check if store is actively referenced in inventory
  const referencedItems = State.inventory.filter(item => item.store === storeName);
  if (referencedItems.length > 0) {
    const reassign = confirm(`This store is actively referenced by ${referencedItems.length} items (e.g. ${referencedItems[0].name}). Would you like to reassign them to "Costco" and delete the store? Clicking cancel keeps the store.`);
    if (reassign) {
      referencedItems.forEach(item => item.store = "Costco");
    } else {
      return; // Keep store
    }
  }
  
  State.customStores.splice(index, 1);
  saveStateToStorage();
  populateStoreDropdowns();
  renderManageStoresList();
};
...
function initMealBuilder() {
  ...
  // Close / Cancel inline editor
  const closeInline = () => {
    document.getElementById("recipe-inline-editor").style.display = "none";
  };
  
  const closeBtn = document.getElementById("btn-recipe-inline-close");
  if (closeBtn) closeBtn.onclick = closeInline;
  
  const cancelBtn = document.getElementById("btn-recipe-edit-cancel");
  ...
  document.getElementById("btn-recipe-edit-save").onclick = () => {
    ...
    document.querySelectorAll(".ingredient-input-row").forEach(row => {
      ...
      const rawQty = qtyEl ? qtyEl.value.trim() : "1/2";
      const ingQty = parseQtyString(rawQty);
      if (ingName) {
        ingredients.push({ name: ingName, qty: ingQty, rawQtyText: rawQty, unit: "%" });
      }
    });
...
function refreshRecipesList() {
  ...
  const list = State.recipes.filter(r => {
    const matchesSearch = 
      ...
      (r.ingredients && r.ingredients.some(ing => ing.name.toLowerCase().includes(query)));
  ...
  if (list.length === 0) {
    container.innerHTML = `
      <div class="empty-state-card" style="text-align: center; padding: 45px 20px; background: var(--bg-card); border-radius: var(--border-radius-md); border: 1.5px dashed var(--border-color); display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; margin-top: 15px;">
        <i class="fas fa-kitchen-set" style="font-size: 40px; color: var(--accent-orange); opacity: 0.65;"></i>
        <div style="font-weight: 700; font-size: 16px; color: var(--text-main);">Your Meal Builder is Empty</div>
        <p style="font-size: 12.5px; color: var(--text-secondary); max-width: 260px; margin: 0 auto; line-height: 1.4;">Create custom meal recipes manually, or pull automated meal recommendations from the AI assistant to populate ideas.</p>
      </div>
    `;
    return;
  }
  ...
  list.forEach(recipe => {
    ...
    card.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 4px; margin-bottom: 2px;">
        <span class="recipe-card-title" style="font-size: 16px; font-weight: 700; line-height: 1.2; color: var(--text-main);">${recipe.name}</span>
      </div>
      
      <p class="recipe-card-desc" style="font-size: 13px; margin-bottom: 6px; color: var(--text-secondary); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.25;">${recipe.description}</p>
      
      <div style="display: flex; flex-direction: column; gap: 3px; margin-bottom: 6px; font-size: 11.5px; color: var(--text-secondary); border-left: 2px solid var(--border-color); padding-left: 6px;">
        <div><strong>Macros:</strong> ${recipe.nutrition.calories}kcal | P:${recipe.nutrition.protein}g | C:${recipe.nutrition.carbs}g | F:${recipe.nutrition.fat}g</div>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 2px;">
          <span><strong>Servings:</strong> ${servingsAvailable > 0 ? `${servingsAvailable}x` : '<span style="color:var(--accent-orange); font-weight:700;">Out</span>'}</span>
          <span><strong>Cost:</strong> <span style="color: var(--primary-green); font-weight: 700;">${formatCurrency(recipeCost)}</span></span>
        </div>
      </div>

      <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 8px; padding-top: 4px; border-top: 1px dashed var(--border-color);">
        <span class="recipe-ingredients-check ${matchClass}" style="font-size: 11px; font-weight: 700; padding: 2px 4px; display: inline-flex; align-items: center; gap: 2px;"><i class="fas ${check.available ? 'fa-circle-check' : 'fa-circle-question'}" style="font-size:10px;"></i> ${matchText}</span>
        <div style="display: flex; gap: 6px; align-items: center;">
          <span class="recipe-ingredients-toggle-btn" style="font-size: 11.5px; font-weight: 700; color: var(--primary-green); cursor: pointer; user-select: none;" onclick="event.stopPropagation(); toggleRecipeDropdown('${recipe.id}', this)">Ingredients &or;</span>
          <i class="fas fa-pen recipe-edit-btn" style="color: var(--primary-green); font-size: 13px; cursor: pointer; padding: 4px;" onclick="event.stopPropagation(); openRecipeEditModal('${recipe.id}')"></i>
          <i class="fas fa-trash recipe-delete-btn" style="color: var(--accent-orange); font-size: 13px; cursor: pointer; padding: 4px;" onclick="event.stopPropagation(); deleteRecipe('${recipe.id}')"></i>
        </div>
      </div>
      
      <div class="recipe-details-dropdown" id="dropdown-${recipe.id}" style="font-size: 11.5px; margin-top: 6px;">
        ...
        <strong style="margin-top: 4px; font-size: 11px; display: block;">Instructions:</strong>
        <p style="margin: 0; font-size: 11px; line-height: 1.2;">${recipe.instructions}</p>
      </div>
    `;
...
function openRecipeEditModal(recipeId = null) {
  const inlineEditor = document.getElementById("recipe-inline-editor");
  ...
  if (recipeId) {
    ...
    document.getElementById("recipe-inline-title").textContent = "Edit Recipe";
    idInput.value = r.id;
    ...
    r.ingredients.forEach(ing => {
      addIngredientEditorRow(ing.name, ing.qty, ing.rawQtyText || "");
    });
  } else {
    document.getElementById("recipe-inline-title").textContent = "Create Recipe";
    idInput.value = "";
    nameInput.value = "";
    descInput.value = "";
    instructionsInput.value = "";
    addIngredientEditorRow("", 0.5, "1/2");
    addIngredientEditorRow("", 0.5, "1/2");
  }
}

function addIngredientEditorRow(name = "", qty = 0.5, rawQtyText = "") {
  const container = document.getElementById("recipe-edit-ingredients-container");
  const row = document.createElement("div");
  row.className = "ingredient-input-row";
  
  let displayValue = rawQtyText;
  if (!displayValue) {
    let qtyVal = parseFloat(qty) || 0.5;
    if (qtyVal === 0.5) displayValue = "1/2";
    else if (qtyVal === 0.25) displayValue = "1/4";
    else if (qtyVal === 0.125) displayValue = "1/8";
    else if (qtyVal === 0.333) displayValue = "1/3";
    else if (qtyVal === 0.667) displayValue = "2/3";
    else if (qtyVal === 0.75) displayValue = "3/4";
    else if (qtyVal === 1.0) displayValue = "100%";
    else displayValue = `${(qtyVal * 100).toFixed(0)}%`;
  }
  
  const uniqueGroceryNames = [...new Set(State.inventory.map(item => item.name))];
  uniqueGroceryNames.sort();
  
  let selectOptions = `<option value="">-- Choose Grocery Item --</option>`;
  uniqueGroceryNames.forEach(gName => {
    selectOptions += `<option value="${gName}" ${gName.toLowerCase() === name.toLowerCase() ? 'selected' : ''}>${gName}</option>`;
  });
  if (name && !uniqueGroceryNames.some(n => n.toLowerCase() === name.toLowerCase())) {
    selectOptions += `<option value="${name}" selected>${name}</option>`;
  }
  
  row.innerHTML = `
    <select class="form-control ing-row-name" style="flex:2.5; background: var(--bg-primary); color: var(--text-main); border: 1px solid var(--border-color); border-radius: var(--border-radius-sm); padding: 4px 8px;">
      ${selectOptions}
    </select>
    <input type="text" class="form-control ing-row-qty" placeholder="e.g. 1/2 or 15%" value="${displayValue}" style="flex:1.5; text-align:center;">
...
function renderAnalyticsCharts() {
  ...
  document.getElementById("stat-anal-total-spend").innerHTML = `${formatCurrency(textSpendVal)}${estLabel}`;
  ...
  const symbol = getCurrencySymbol();
  const spendData = State.trips.length > 0 ? [195.00, 245.20, textSpendVal] : [0, 0, 0];
  const limitData = State.trips.length > 0 ? [300, 300, State.user.budgetAmount] : [0, 0, 0];
  ...
  const ratioData = (textSpendVal === 0 && totalWastedVal === 0) ? [0, 0] : [textSpendVal, totalWastedVal];
  ...
  // Redo Nutrition Goal Met with 4 visually stunning macro radial progress rings
  const nutritionContainer = document.getElementById("nutrition-rings-container");
  if (nutritionContainer) {
    const goals = State.user.macroGoals || { calories: 2000, protein: 120, carbs: 200, fat: 70 };
    ...
    if (State.calendar) {
      for (let dateKey in State.calendar) {
        for (let slotKey in State.calendar[dateKey]) {
          const slot = getCalendarMeal(State.calendar[dateKey][slotKey]);
          if (slot && slot.status === "Eaten") {
            const recipe = State.recipes.find(r => r.id === slot.recipeId);
            if (recipe) {
              eatenCount++;
              const nut = recipe.nutrition || {};
              actualCalories += nut.calories || 650;
              actualProtein += nut.protein || 35;
              actualCarbs += nut.carbs || 60;
              actualFat += nut.fat || 22;
            }
          }
        }
      }
    ...
    // Start at zero if no foods are scheduled and eaten yet
    if (eatenCount === 0) {
      actualCalories = 0;
      actualProtein = 0;
      actualCarbs = 0;
    ...
    const macros = [
      { name: "Calories", actual: actualCalories, target: goals.calories, unit: "kcal", color: "var(--primary-green)" },
      { name: "Protein", actual: actualProtein, target: goals.protein, unit: "g", color: "var(--freezer-blue)" },
      { name: "Carbs", actual: actualCarbs, target: goals.carbs, unit: "g", color: "#f3722c" },
      { name: "Fats", actual: actualFat, target: goals.fat, unit: "g", color: "#ef476f" }
    ];
    
    let html = `<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; width: 100%;">`;
    
    macros.forEach(macro => {
      const percent = Math.min(100, Math.round((macro.actual / macro.target) * 100));
      const strokeDash = `${percent} 100`;
      
      html += `
        <div class="macro-ring-card" style="background: var(--bg-card); border-radius: var(--border-radius-md); padding: 18px 12px; display: flex; flex-direction: column; align-items: center; border: 1px solid var(--border-color); box-shadow: var(--box-shadow-sm);">
          <div style="position: relative; width: 85px; height: 85px; display: flex; align-items: center; justify-content: center;">
            <svg width="85" height="85" viewBox="0 0 36 36" style="transform: rotate(-90deg); width: 100%; height: 100%;">
              <circle cx="18" cy="18" r="15.915" fill="none" stroke="var(--border-color)" stroke-width="3.5" style="opacity: 0.15;" />
              <circle cx="18" cy="18" r="15.915" fill="none" stroke="${macro.color}" stroke-width="3.5" stroke-dasharray="${strokeDash}" stroke-linecap="round" style="transition: stroke-dasharray 0.6s cubic-bezier(0.4, 0, 0.2, 1);" />
            </svg>
            <div style="position: absolute; text-align: center;">
              <span style="font-size: 15px; font-weight: 800; color: var(--text-main);">${percent}%</span>
            </div>
          </div>
          <span style="font-weight: 700; font-size: 12.5px; margin-top: 12px; color: var(--text-main); text-align: center;">${macro.name}</span>
          <span style="font-size: 11px; color: var(--text-secondary); margin-top: 3px; font-weight: 500;">${Math.round(macro.actual)} / ${macro.target} ${macro.unit}</span>
        </div>
      `;
    });
...
function initProfileSettings() {
  ...
  document.getElementById("btn-cropper-save").onclick = () => {
    ...
    try {
      ...
      console.warn("Canvas crop failed (CORS or canvas exception), falling back to raw image src:", err);
      croppedBase64 = cropperImg.src;
    }
  ...
  document.getElementById("btn-logout").onclick = () => {
    if (confirm("Are you sure you want to sign out? Data will remain stored safely in the cloud.")) {
      localStorage.setItem("grocerly_logged_in", "false");
      localStorage.removeItem("grocerly_state_cache");
      
      // Reset State to clean initial values
      State.user = {
        name: "Jane Doe",
        email: "jane.doe@gmail.com",
        householdSize: 2,
  ...
  // Admin Mode Toggle Bind
  const adminToggle = document.getElementById("admin-mode-toggle");
  if (adminToggle) {
    adminToggle.addEventListener("change", () => {
      State.user.isAdmin = adminToggle.checked;
      saveStateToStorage();
      ClientAPI.syncBackend();
      refreshProfileCard();
      if (adminToggle.checked) {
        showToast("Admin Mode Enabled! Extra controls unlocked.", "success");
      } else {
  ...
  // Feedback Modal Binds
  const btnFeedback = document.getElementById("btn-profile-submit-feedback");
  const modalFeedback = document.getElementById("modal-submit-feedback");
  const closeFeedback = document.getElementById("btn-feedback-modal-close");
  const saveFeedback = document.getElementById("btn-submit-feedback-save");
  ...
  if (closeFeedback && modalFeedback) {
    closeFeedback.onclick = () => {
      modalFeedback.classList.remove("active");
    };
  }

  if (saveFeedback && modalFeedback) {
    saveFeedback.onclick = () => {
      ...
      .then(() => {
        showToast("Thank you for your feedback! It was sent successfully.", "success");
        modalFeedback.classList.remove("active");
        
        // If they are admin, reload the feedback list in the admin panel automatically
        if (State.user.isAdmin) {
          refreshAdminDashboard();
        }
      })
...
function refreshProfileCard() {
  ...
  const curSelect = document.getElementById("profile-currency-select");
  if (curSelect && State.user.currency) {
    curSelect.value = State.user.currency;
  }
  ...
  // Admin Toggle and Mock Control Panel visibility
  ...
  if (adminContainer) {
    adminContainer.style.display = isJirar ? "flex" : "none";
  }
  if (adminToggle) {
    adminToggle.checked = !!State.user.isAdmin;
  }
  if (adminPanel) {
    adminPanel.style.display = (isJirar && State.user.isAdmin) ? "block" : "none";
  }
  const navItemAdmin = document.getElementById("nav-item-admin");
  if (navItemAdmin) {
    navItemAdmin.style.display = (isJirar && State.user.isAdmin) ? "flex" : "none";
...
function renderGlobalSearchResults(term) {
  ...
  State.trips.forEach(trip => {
    if (trip.store.toLowerCase().includes(query)) {
      results.push({ name: `${trip.store} Shopping Trip`, subtitle: `${trip.date} • Total Spent: ${formatCurrency(trip.cost)}`, type: "Shopping History", action: () => {
        alert(`Shopping Trip Log Detail:\nStore: ${trip.store}\nDate: ${trip.date}\nTotal Cost: ${formatCurrency(trip.cost)}\nItems: ${trip.items.join(", ")}`);
      }});
    }
  });
...
function saveStateToStorage() {
  try {
    ...
    console.warn("localStorage quota exceeded, saving state without image preview to avoid crash:", e);
    // Fallback: save State but temporarily strip huge avatar from local storage cache to keep database working
    const cloned = JSON.parse(JSON.stringify(State));
    if (cloned.user && cloned.user.avatar && cloned.user.avatar.length > 50000) {
      cloned.user.avatar = ""; // strip heavy base64
    }
    try {
      localStorage.setItem("grocerly_state_cache", JSON.stringify(cloned));
    } catch (innerErr) {
      console.error("Critical local storage write failure", innerErr);
    }
  }
...
function loadStateFromStorage() {
  ...
  if (cached) {
    try {
      ...
      if (!State.user.shoppingLists) State.user.shoppingLists = {};
      ...
      // Update currency dropdown value from state
      if (State.user && State.user.currency) {
        const curSelect = document.getElementById("profile-currency-select");
        if (curSelect) curSelect.value = State.user.currency;
      }
    } catch (e) {
...
const MockBackend = {
  getDb() {
    ...
    // Ensure we have a high-fidelity set of mock users for investor pitch deck presentations
    if (parsedDb && parsedDb.users && Object.keys(parsedDb.users).length < 5) {
      const mockProfiles = {
        "mark.johnson@gmail.com": {
          ...
          profile: {
            ...
            budgetAmount: 450,
            desiredStorageWeeks: 2,
            lastSync: new Date(Date.now() - 3 * 3600000).toISOString() // 3 hrs ago (Active)
          },
        ...
        "sarah.kim@outlook.com": {
          ...
          profile: {
            ...
            lastSync: new Date(Date.now() - 1 * 3600000).toISOString() // 1 hr ago (Active)
          },
        ...
        "alex.garcia@yahoo.com": {
          ...
          profile: {
            ...
            lastSync: new Date(Date.now() - 12 * 3600000).toISOString() // 12 hrs ago (Active)
          },
        ...
        "elizabeth.smith@gmail.com": {
          ...
          profile: {
            ...
            budgetAmount: 380,
            desiredStorageWeeks: 2,
            lastSync: new Date(Date.now() - 96 * 3600000).toISOString() // 4 days ago (Offline)
          },
        ...
        "guest@grocerly.app": {
          ...
          profile: {
            ...
            budgetAmount: 250,
            desiredStorageWeeks: 2,
            lastSync: new Date().toISOString() // Now (Active)
          },
      ...
      for (const k in mockProfiles) {
        if (!parsedDb.users[k]) {
          parsedDb.users[k] = mockProfiles[k];
        }
      }
      localStorage.setItem("grocerly_local_db", JSON.stringify(parsedDb));
    }
    return parsedDb;
  },
  ...
  async handleRequest(endpoint, method, body) {
    ...
    if (path === "/api/auth/config" && method === "GET") {
      return { clientId: "721951712629-f4i7m3j2vk4bkana2vjbap26res637g3.apps.googleusercontent.com" };
    }
    ...
    if (path === "/api/auth/google" && method === "POST") {
      ...
      let name = "Jane Doe";
      let picture = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23a0aec0"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
      let sub = null;
      ...
      if (credential && credential.includes('@')) {
        ...
        sub = email;
      } else if (credential && credential.includes('.') && credential.split('.').length === 3) {
        ...
        try {
          ...
          console.warn("Mock JWT decode failed, using default info");
        }
      }

      const googleUid = sub || email;

      if (!db.users[googleUid]) {
        db.users[googleUid] = {
    ...
    if (path === "/api/admin/users" && method === "GET") {
      ...
      for (const k in db.users) {
        ...
        list.push({
          ...
          inventoryCount: userObj.inventory ? userObj.inventory.length : 0,
          recipesCount: userObj.recipes ? userObj.recipes.length : 0,
          tripsCount: userObj.trips ? userObj.trips.length : 0,
    ...
    if (path === "/api/feedback" && method === "POST") {
      ...
      db.feedback.push(f);
      this.saveDb(db);
      return { success: true, feedback: f };
    }

    const email = queryParams["email"] || (body && body.email);
    if (!email) {
    ...
    let userKey = email;
    if (!db.users[userKey]) {
      // Find user by profile email (fallback for Google login googleUid keys)
      const foundKey = Object.keys(db.users).find(k => db.users[k].profile && db.users[k].profile.email === email);
      if (foundKey) {
        userKey = foundKey;
      } else {
        throw new Error("User not found or unauthorized");
    ...
    if (path === "/api/calendar") {
      if (method === "GET") return user.calendar || {};
      if (method === "PUT") {
        user.calendar = body.calendar;
        this.saveDb(db);
        return user.calendar;
      }
    }

    if (path === "/api/custom-stores") {
      if (method === "GET") return user.customStores || [];
      if (method === "PUT") {
        user.customStores = body.customStores;
        this.saveDb(db);
        return user.customStores;
      }
    }
...
const ClientAPI = {
  async request(endpoint, method = 'GET', body = null) {
    ...
    if (isWrite && !endpoint.includes('/api/auth')) {
    ...
    try {
      ...
      if (isWrite && !endpoint.includes('/api/auth')) {
        setTimeout(() => updateCloudSyncBadge("online"), 600); // short visual delay
      }
      return data;
    } catch (e) {
  ...
  async syncBackend() {
    ...
    try {
      // Sync profile
      await this.request('/api/profile', 'PUT', { email: State.user.email, userId: uid, profile: State.user });
      // Sync inventory
      await this.request('/api/inventory', 'PUT', { email: State.user.email, userId: uid, inventory: State.inventory });
      // Sync recipes
      await this.request('/api/recipes', 'PUT', { email: State.user.email, userId: uid, recipes: State.recipes });
      // Sync trips
      await this.request('/api/trips', 'PUT', { email: State.user.email, userId: uid, trips: State.trips });
      // Sync waste logs
      await this.request('/api/waste', 'PUT', { email: State.user.email, userId: uid, wasteLogs: State.wasteLogs });
      // Sync notifications
      await this.request('/api/notifications', 'PUT', { email: State.user.email, userId: uid, notifications: State.notifications });
      // Sync calendar
      await this.request('/api/calendar', 'PUT', { email: State.user.email, userId: uid, calendar: State.calendar });
      // Sync custom stores
      await this.request('/api/custom-stores', 'PUT', { email: State.user.email, userId: uid, customStores: State.customStores });
      setTimeout(() => updateCloudSyncBadge("online"), 600);
    } catch (e) {
...
async function loadStateFromServer() {
  ...
  if (!email) return;
  const identifier = State.user.uid || email;
  const param = `?userId=${encodeURIComponent(identifier)}`;
  try {
    const profile = await ClientAPI.request(`/api/profile${param}`);
    const inventory = await ClientAPI.request(`/api/inventory${param}`);
    const recipes = await ClientAPI.request(`/api/recipes${param}`);
    const trips = await ClientAPI.request(`/api/trips${param}`);
    const waste = await ClientAPI.request(`/api/waste${param}`);
    const notifications = await ClientAPI.request(`/api/notifications${param}`);
    const calendar = await ClientAPI.request(`/api/calendar${param}`);
    const customStores = await ClientAPI.request(`/api/custom-stores${param}`);
    ...
    State.customStores = customStores || [];
...
function importForecastedShoppingTrip() {
  ...
  document.getElementById("modal-ai-assistant").classList.remove("active");
  alert(`Imported ${importedCount} forecasted ingredients into inventory! logged trip for ${formatCurrency(totalCost)}.`);
}
...
function generateMealIdea() {
  ...
  // Show and populate inline editor
  openRecipeEditModal();
  document.getElementById("recipe-inline-title").textContent = "Generated Recipe Idea";
  document.getElementById("edit-recipe-id").value = "";
  document.getElementById("edit-recipe-name").value = mealName;
  document.getElementById("edit-recipe-desc").value = "AI generated meal idea using inventory ingredients.";
  document.getElementById("edit-recipe-instructions").value = "1. Prepare your ingredients.\n2. Combine and cook until done.\n3. Serve hot!";
  
  // Populate ingredients rows
  const rowsContainer = document.getElementById("recipe-edit-ingredients-container");
  if (rowsContainer) {
    rowsContainer.innerHTML = "";
    selected.forEach(item => {
      addIngredientEditorRow(item.name, 0.5, "%");
    });
  }
...
window.refreshPurchasesFolders = function() {
  ...
  if (typeof refreshShoppingListsUI === 'function') {
    ...
    receipts.forEach(receipt => {
      ...
      row.innerHTML = `
        ...
        <div style="display:flex; align-items:center; gap:10px;">
          <span class="receipt-row-cost">${formatCurrency(receipt.cost)}</span>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnIdeasGlobal) {
    btnIdeasGlobal.addEventListener('click', () => {
      const section = document.getElementById("meals-ideas-section");
      if (section) {
        if (section.style.display === "none") {
          section.style.display = "block";
          generateMealBuilderIdeas();
  ...
  const btnIdeasClose = document.getElementById("btn-meals-ideas-close");
  if (btnIdeasClose) {
    btnIdeasClose.addEventListener("click", () => {
      const section = document.getElementById("meals-ideas-section");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnDoneCalc) {
    btnDoneCalc.addEventListener('click', () => {
      ...
      resText.innerText = `Your total at ${store} is ${formatCurrency(total)}`;
...
function generateMealBuilderIdeas() {
  const container = document.getElementById("ai-ideas-content");
  const countBadge = document.getElementById("ai-ideas-count");
  if (!container) return;
  
  container.innerHTML = `<p style='text-align:center; padding:10px; font-size:12px; color:var(--text-secondary);'><i class='fas fa-spinner fa-spin' style='margin-right:6px;'></i>Running backend ideas search engine...</p>`;
  
  // Make real backend API call
  const uid = State.user.uid || State.user.email;
  const param = `?userId=${encodeURIComponent(uid)}`;
  
  ClientAPI.request(`/api/ai/ideas${param}`)
    .then(data => {
      const matches = data.matches || [];
      renderIdeas(matches);
    })
    .catch(err => {
      console.warn("Backend ideas search failed, falling back to client-side online engine:", err);
      
      const activeInventory = State.inventory.filter(item => item.currentQty > 0 && item.remainingPercent > 0);
      const activeNames = activeInventory.map(item => item.name.toLowerCase());
      
      const terms = activeNames.length > 0 
        ? activeNames.map(name => name.split(" ")[0]).slice(0, 3) 
        : ["chicken", "egg", "beef"];

      // Fetch from MealDB in browser
      Promise.all(terms.map(term => {
        if (term.length < 3) return Promise.resolve(null);
        return fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${encodeURIComponent(term)}`)
          .then(res => res.json())
          .catch(() => null);
      })).then(results => {
        let apiMeals = [];
        results.forEach(res => {
          if (res && res.meals) {
            apiMeals = apiMeals.concat(res.meals);
          }
        });
        ...
        if (uniqueMeals.length > 0) {
          uniqueMeals.slice(0, 6).forEach(m => {
            const name = m.strMeal;
            let category = "Dinner";
            const catLower = (m.strCategory || "").toLowerCase();
            const nameLower = name.toLowerCase();
            if (catLower.includes("breakfast") || nameLower.includes("egg") || nameLower.includes("toast")) {
              category = "Breakfast";
            } else if (catLower.includes("salad") || catLower.includes("sandwich") || nameLower.includes("wrap") || nameLower.includes("lunch")) {
              category = "Lunch";
            }
            
            const description = `Delicious online recipe for ${name}. Category: ${m.strCategory || 'General'}`;
            
            const ingredients = [];
            for (let i = 1; i <= 20; i++) {
              const ingName = m[`strIngredient${i}`];
              if (ingName && ingName.trim()) {
                const measure = m[`strMeasure${i}`] || "1 unit";
                ingredients.push({
                  name: ingName.trim(),
                  qty: 1,
                  unit: measure.trim()
            ...
            matches.push({
              ...
              description,
              ingredients,
              instructions: m.strInstructions || "Cook according to online instructions.",
              nutrition: {
        ...
        // If no internet matches, fallback to local templates
        if (matches.length === 0) {
          const templates = [
            {
              name: "Avocado Spinach Omit",
              category: "Breakfast",
              description: "Fluffy eggs folded with spinach, sliced avocado, and cheese.",
              ingredients: [
                { name: "Organic Eggs", qty: 2, unit: "pcs" },
                { name: "Organic Spinach", qty: 0.25, unit: "bag" },
            ...
            },
            {
              ...
              category: "Breakfast",
              description: "Classic toasted breakfast sandwich topped with scrambled eggs.",
              ingredients: [
                { name: "Organic Eggs", qty: 2, unit: "pcs" }
              ],
              instructions: "Scramble eggs, toast bread, build sandwich.",
              nutrition: { calories: 380, protein: 20, fat: 16, carbs: 36 }
            },
            {
              name: "Chicken Spinach Salad",
              category: "Lunch",
              description: "Warm chicken breast strips and fresh spinach.",
              ingredients: [
                { name: "Organic Chicken Breast", qty: 1, unit: "pcs" }
              ],
              instructions: "Warm chicken, roll inside wrap with spinach.",
  ...
  function renderIdeas(matches) {
    container.innerHTML = "";
    if (matches.length === 0) {
      container.innerHTML = `<p style='text-align:center; padding:10px; font-size:12px; color:var(--text-secondary);'>No matching ingredients found. Add items to My Groceries to see ideas!</p>`;
      if (countBadge) countBadge.textContent = "0 Ideas";
      return;
    }

    if (countBadge) countBadge.textContent = `${matches.length} Idea${matches.length !== 1 ? 's' : ''}`;
    ...
    categories.forEach(cat => {
      ...
      catMatches.forEach(m => {
        ...
        const missingHtml = m.missingIngs.length > 0 
          ? `<div style="font-size:10px; color:var(--accent-orange); font-weight:600;"><i class="fas fa-triangle-exclamation"></i> Missing: ${m.missingIngs.join(", ")}</div>`
          : `<div style="font-size:10px; color:var(--primary-green); font-weight:700;"><i class="fas fa-circle-check"></i> All ingredients in stock!</div>`;
        
        const recipeDataStr = encodeURIComponent(JSON.stringify(m));

        card.innerHTML = `
          <div style="font-weight: 700; font-size: 13px; color: var(--text-main); margin-bottom: 2px;">${m.name}</div>
          <p style="font-size: 11px; color: var(--text-secondary); margin: 0; line-height: 1.3;">${m.description}</p>
          <div style="font-size: 10px; color: var(--text-secondary);">
            Uses: ${m.matchingIngs.map(name => `<strong style="color:var(--text-main);">${name}</strong>`).join(", ")}
          </div>
          ${missingHtml}
          <div style="display: flex; gap: 6px; margin-top: auto; padding-top: 6px; border-top: 1px dashed var(--border-color);">
            <button class="btn btn-primary" onclick="saveAiIdeaDirect('${recipeDataStr}')" style="font-size: 11px; padding: 4px 8px; height: auto; flex: 1.5; font-weight: 700;">Add to Meal Builder</button>
            <button class="btn btn-outline" onclick="editAiIdeaInline('${recipeDataStr}')" style="font-size: 11px; padding: 4px 8px; height: auto; flex: 1; border-color: var(--primary-green); color: var(--primary-green);">Customize</button>
          </div>
        `;
...
window.saveAiIdeaDirect = function(recipeDataStr) {
  try {
    const m = JSON.parse(decodeURIComponent(recipeDataStr));
    const newRecipe = {
      id: `rec_${Date.now()}`,
      name: m.name,
    ...
    saveStateToStorage();
    refreshRecipesList();
    alert(`"${m.name}" has been saved to your recipes!`);
  } catch (e) {
    console.error("Failed to save AI idea", e);
  }
};

window.editAiIdeaInline = function(recipeDataStr) {
  try {
    const m = JSON.parse(decodeURIComponent(recipeDataStr));
    
    // Open recipe inline editor
    const editor = document.getElementById("recipe-inline-editor");
    if (!editor) return;
    
    editor.style.display = "block";
    document.getElementById("recipe-inline-title").textContent = "Customize Recipe Idea";
    document.getElementById("edit-recipe-id").value = ""; // new recipe
    document.getElementById("edit-recipe-name").value = m.name;
    document.getElementById("edit-recipe-desc").value = m.description;
    document.getElementById("edit-recipe-instructions").value = m.instructions;
    ...
    console.error("Failed to customize AI idea", e);
  }
};


window.adminFillMockPantry = function() {
  const mockItems = [
    { id: "mock_g_1", name: "Organic Eggs", category: "Dairy & Eggs", storageLoc: "Fridge", currentQty: 12, unit: "pcs", price: 4.99, purchaseDate: "2026-07-01", expiryDate: "2026-07-25", remainingPercent: 100, timesUsed: 0 },
    { id: "mock_g_2", name: "Organic Chicken Breast", category: "Meat & Seafood", storageLoc: "Freezer", currentQty: 2, unit: "lbs", price: 12.50, purchaseDate: "2026-07-02", expiryDate: "2026-12-30", remainingPercent: 100, timesUsed: 0 },
    { id: "mock_g_3", name: "Whole Milk 1gal", category: "Dairy & Eggs", storageLoc: "Fridge", currentQty: 1, unit: "gal", price: 3.89, purchaseDate: "2026-07-03", expiryDate: "2026-07-15", remainingPercent: 100, timesUsed: 0 },
    { id: "mock_g_4", name: "Avocados", category: "Fruits & Vegetables", storageLoc: "Pantry", currentQty: 4, unit: "pcs", price: 5.00, purchaseDate: "2026-07-04", expiryDate: "2026-07-12", remainingPercent: 100, timesUsed: 0 },
    { id: "mock_g_5", name: "Organic Spinach", category: "Fruits & Vegetables", storageLoc: "Fridge", currentQty: 1, unit: "bag", price: 2.99, purchaseDate: "2026-07-04", expiryDate: "2026-07-10", remainingPercent: 100, timesUsed: 0 }
  ];
  State.inventory = mockItems;
  saveStateToStorage();
  ClientAPI.syncBackend();
  refreshDashboard();
  refreshInventoryList();
  showToast("Mock Pantry Filled successfully!", "success");
};

window.adminClearPantry = function() {
  State.inventory = [];
  saveStateToStorage();
  ClientAPI.syncBackend();
  refreshDashboard();
...
window.adminSimulatePurchase = function() {
  const storeNames = ["Trader Joe's", "Whole Foods", "Costco", "ShopRite"];
  const randomStore = storeNames[Math.floor(Math.random() * storeNames.length)];
  const cost = parseFloat((Math.random() * 45 + 15).toFixed(2));
  const newTrip = {
    id: `trip_${Date.now()}`,
    store: randomStore,
    cost: cost,
    date: new Date().toISOString().split("T")[0],
    items: ["Apples", "Chicken Strips", "Bread", "Yogurt"]
  };
...
function refreshAdminDashboard() {
  const usersCountEl = document.getElementById("admin-stat-users-count");
  const activeRateEl = document.getElementById("admin-stat-active-rate");
  const feedbackCountEl = document.getElementById("admin-stat-feedback-count");
  const usersTableBody = document.getElementById("admin-users-table-body");
  const feedbackContainer = document.getElementById("admin-feedback-list-container");

  // Pitch Metrics elements
  const avgAgeEl = document.getElementById("admin-stat-avg-age");
  const avgHouseholdEl = document.getElementById("admin-stat-avg-household");
  const totalBudgetEl = document.getElementById("admin-stat-total-budget");
  const wastePreventedEl = document.getElementById("admin-stat-waste-prevented");
  const geoListEl = document.getElementById("admin-geographic-list");
  
  // Engagement elements
  const engPantryEl = document.getElementById("admin-engagement-pantry-count");
  const engRecipesEl = document.getElementById("admin-engagement-recipes-count");
  const engTripsEl = document.getElementById("admin-engagement-trips-count");
  const engRatioEl = document.getElementById("admin-engagement-saved-ratio");

  if (usersCountEl) usersCountEl.textContent = "...";
  if (activeRateEl) activeRateEl.textContent = "...";
  if (feedbackCountEl) feedbackCountEl.textContent = "...";
  
  // 1. Fetch Users
  ClientAPI.request('/api/admin/users')
    .then(data => {
      ...
      if (activeRateEl) activeRateEl.textContent = `${activeCount} Active / ${offlineCount} Off`;
      
      // A. Average User Age
      const usersWithAge = users.filter(u => u.age !== null && u.age > 0);
      const avgAge = usersWithAge.length > 0 
        ? (usersWithAge.reduce((sum, u) => sum + u.age, 0) / usersWithAge.length).toFixed(1) 
        : "N/A";
      if (avgAgeEl) avgAgeEl.textContent = avgAge !== "N/A" ? `${avgAge} Yrs` : "N/A";

      // B. Average Household Size
      const avgHousehold = users.length > 0
        ? (users.reduce((sum, u) => sum + u.householdSize, 0) / users.length).toFixed(1)
        : "N/A";
      if (avgHouseholdEl) avgHouseholdEl.textContent = avgHousehold !== "N/A" ? `${avgHousehold} Ppl` : "N/A";

      // C. Total Budget Managed (Annualized)
      const totalMonthlyBudget = users.reduce((sum, u) => sum + u.budgetAmount, 0);
      const totalAnnualBudget = totalMonthlyBudget * 12;
      if (totalBudgetEl) totalBudgetEl.textContent = formatCurrency(totalAnnualBudget);

      // D. Est. Waste Prevented (ESG Valuation: 3.75% of budget saved from waste)
      const wasteSaved = totalAnnualBudget * 0.0375;
      if (wastePreventedEl) wastePreventedEl.textContent = formatCurrency(wasteSaved);
      ...
      if (geoListEl) {
        ...
        const locMap = {};
        users.forEach(u => {
          const loc = u.location && u.location.toLowerCase() !== "unknown" && u.location.trim() ? u.location : "Local Sandbox";
          locMap[loc] = (locMap[loc] || 0) + 1;
        });

        const sortedLocs = Object.keys(locMap).sort((a, b) => locMap[b] - locMap[a]);
        if (sortedLocs.length === 0) {
          geoListEl.innerHTML = `<p style="font-size:11px; color:var(--text-secondary); text-align:center;">No locations logged.</p>`;
        } else {
          sortedLocs.forEach(loc => {
            ...
            row.innerHTML = `
              <span style="color:var(--text-secondary); font-weight:600;"><i class="fas fa-map-location-dot" style="color: var(--freezer-blue); margin-right:4px;"></i> ${loc}</span>
              <span class="recipe-tag" style="background-color: var(--border-color); color: var(--text-main); font-size:10px; padding: 1px 6px; border:none;">${count} user${count !== 1 ? 's' : ''}</span>
            `;
      ...
      // F. Pantry App Engagement Metrics
      const totalPantry = users.reduce((sum, u) => sum + u.inventoryCount, 0);
      const totalRecipes = users.reduce((sum, u) => sum + u.recipesCount, 0);
      const totalTrips = users.reduce((sum, u) => sum + u.tripsCount, 0);

      if (engPantryEl) engPantryEl.textContent = `${totalPantry} Items`;
      if (engRecipesEl) engRecipesEl.textContent = `${totalRecipes} Recipes`;
      if (engTripsEl) engTripsEl.textContent = `${totalTrips} Trips`;
      
      // Calculate active sustainable ratio (total pantry items used / total items tracked)
      if (engRatioEl) {
        engRatioEl.textContent = totalPantry > 0 ? `${((totalRecipes + totalTrips) / (totalPantry + 1) * 85).toFixed(1)}%` : "85.2%";
      }

      // G. Render User Database activity table
      if (usersTableBody) {
        ...
        users.forEach(u => {
          ...
          const userAvatar = u.avatar || 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23a0aec0"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
          row.innerHTML = `
            <td style="padding: 10px 6px; font-weight:600; display:flex; align-items:center; gap:8px; border-bottom:none;">
              <img src="${userAvatar}" style="width: 24px; height: 24px; border-radius: 50%; object-fit: cover; border: 1.5px solid var(--border-color);" />
              <span>${u.name}</span>
            </td>
            <td style="padding: 10px 6px; color: var(--text-secondary);">${u.email}</td>
            <td style="padding: 10px 6px;">${u.householdSize} Member${u.householdSize !== 1 ? 's' : ''}</td>
            <td style="padding: 10px 6px; font-family: monospace;">${u.lastSync}</td>
            <td style="padding: 10px 6px; text-align: right;">
              <span class="recipe-tag" style="background-color: ${u.isActive ? 'var(--primary-green-light)' : 'var(--border-color)'}; color: ${u.isActive ? 'var(--primary-green)' : 'var(--text-secondary)'}; border:none; padding: 2px 6px; font-size: 10px;">
  ...
  // 2. Fetch Feedback
  ClientAPI.request('/api/admin/feedback')
    .then(data => {
      ...
      if (feedbackContainer) {
        ...
        if (feedback.length === 0) {
          feedbackContainer.innerHTML = `<p style="font-size:12px; color:var(--text-secondary); text-align:center;">No feedback submissions yet.</p>`;
          return;
        }
        ...
        feedback.forEach(f => {
          ...
          card.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center;">
              <span style="font-weight:700; font-size:13px; color: var(--text-main);">${f.name} <span style="font-weight:normal; font-size:11px; color:var(--text-secondary);">(${f.email})</span></span>
              <span style="font-size:10px; color:var(--text-secondary);">${f.date}</span>
            </div>
            <p style="font-size:12px; margin: 4px 0 0 0; color:var(--text-main); line-height: 1.4;">"${f.message}"</p>
          `;
    ...
    .catch(err => {
      ...
      if (feedbackCountEl) feedbackCountEl.textContent = "Error";
    });
}


/* ==================================================== */
/* DIGITAL SHOPPING LIST CHECKLIST SYSTEM */
/* ==================================================== */

window.exportInventoryToShoppingLists = function() {
  if (!State.user) State.user = {};
  if (!State.user.shoppingLists) {
    State.user.shoppingLists = {};
  }
  
  // Group all ingredients by store
  const itemsByStore = {};
  State.inventory.forEach(item => {
    const storeName = item.store ? item.store.trim() : "Other Stores";
    if (!itemsByStore[storeName]) {
      itemsByStore[storeName] = [];
    }
    
    // Check if item is already in list (avoid duplicates)
    const exists = itemsByStore[storeName].some(existing => existing.name.toLowerCase() === item.name.toLowerCase());
    if (!exists) {
  ...
  // Merge: Keep checked states of existing items, append new ones
  for (const storeName in itemsByStore) {
    const currentList = State.user.shoppingLists[storeName] || [];
    const newList = [...currentList];
    
    itemsByStore[storeName].forEach(newItem => {
      const exists = newList.some(item => item.name.toLowerCase() === newItem.name.toLowerCase());
      if (!exists) {
        newList.push(newItem);
      }
    });
    State.user.shoppingLists[storeName] = newList;
  }
  
  saveStateToStorage();
  refreshShoppingListsUI();
  
  alert("Success: Exported ingredients to store-specific digital shopping lists under the 'Purchases' tab!");
};

window.refreshShoppingListsUI = function() {
  const grid = document.getElementById("shopping-lists-grid");
  if (!grid) return;
  grid.innerHTML = "";

  const shoppingLists = State.user.shoppingLists || {};
  const storeNames = Object.keys(shoppingLists).filter(name => shoppingLists[name].length > 0);

  if (storeNames.length === 0) {
    grid.innerHTML = `
      <div style="grid-column: 1 / -1; text-align: center; padding: 25px 15px; background: rgba(255,255,255,0.01); border-radius: var(--border-radius-md); border: 1.5px dashed var(--border-color); display: flex; flex-direction: column; align-items: center; gap: 8px;">
        <i class="fas fa-cart-flatbed-suitcases" style="font-size: 28px; color: var(--primary-green); opacity: 0.55;"></i>
        <div style="font-weight: 700; font-size: 13.5px; color: var(--text-main);">No Digital Shopping Lists Yet</div>
        <p style="font-size: 11px; color: var(--text-secondary); max-width: 280px; margin: 0 auto; line-height: 1.35;">
          Click the <strong>"Export Lists"</strong> button on the <strong>Groceries</strong> tab to automatically create checklists from your ingredients, or add a purchase receipt.
        </p>
      </div>
    `;
    return;
  ...
  storeNames.forEach(storeName => {
    const list = shoppingLists[storeName];
    const checkedCount = list.filter(item => item.checked).length;
    
    const card = document.createElement("div");
    card.className = "receipt-folder-card";
    ...
    else iconClass = "fa-cart-shopping";
    
    card.innerHTML = `
      <i class="fas ${iconClass} receipt-folder-icon" style="color: var(--primary-green);"></i>
      <div class="receipt-folder-name">${storeName}</div>
      <div class="receipt-folder-count">${list.length} Item${list.length !== 1 ? 's' : ''} (${checkedCount} checked)</div>
    `;
    card.onclick = () => openShoppingListModal(storeName);
    grid.appendChild(card);
  });
};

window.openShoppingListModal = function(storeName) {
  const modal = document.getElementById("modal-shopping-list");
  if (!modal) return;
  
  modal.dataset.storeName = storeName;
  document.getElementById("shopping-list-modal-title").textContent = `${storeName} Shopping List`;
  
  renderShoppingListItems(storeName);
  modal.classList.add("active");
};

function renderShoppingListItems(storeName) {
  ...
  if (list.length === 0) {
    container.innerHTML = `<p style="font-size: 12px; color: var(--text-secondary); text-align: center; padding: 20px 0;">This checklist is empty. Add a new item below!</p>`;
    return;
  }
  ...
  list.forEach(item => {
    ...
    const textOpacity = item.checked ? "0.6" : "1";
    
    row.innerHTML = `
      <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; user-select: none; flex: 1; margin: 0;">
        <input type="checkbox" ${item.checked ? "checked" : ""} style="width: 18px; height: 18px; accent-color: var(--primary-green); margin: 0; cursor: pointer;">
        <span style="font-size: 13.5px; font-weight: 500; text-decoration: ${textDecoration}; opacity: ${textOpacity}; color: var(--text-main);">${item.name} <span style="font-size:10px; color: var(--text-secondary); font-weight:normal;">(${item.category || "Pantry"})</span></span>
      </label>
      <button style="background: transparent; border: none; color: var(--accent-orange); cursor: pointer; padding: 4px 8px; font-size: 13px;" title="Delete item">
    ...
    deleteBtn.onclick = (e) => {
      e.stopPropagation();
      deleteShoppingListItem(storeName, item.id);
    };
...
window.deleteShoppingListItem = function(storeName, itemId) {
  const shoppingLists = State.user.shoppingLists || {};
  const list = shoppingLists[storeName] || [];
...
window.addShoppingListItem = function(storeName, name) {
  ...
  let matchedCategory = "Pantry";
  const nameLower = cleanName.toLowerCase();
  if (nameLower.includes("chicken") || nameLower.includes("beef") || nameLower.includes("pork") || nameLower.includes("turkey") || nameLower.includes("fish") || nameLower.includes("salmon") || nameLower.includes("meat")) {
    matchedCategory = "Meat & Poultry";
  } else if (nameLower.includes("egg") || nameLower.includes("milk") || nameLower.includes("cheese") || nameLower.includes("butter") || nameLower.includes("yogurt") || nameLower.includes("dairy")) {
    matchedCategory = "Dairy & Eggs";
  } else if (nameLower.includes("apple") || nameLower.includes("banana") || nameLower.includes("spinach") || nameLower.includes("lettuce") || nameLower.includes("tomato") || nameLower.includes("avocado") || nameLower.includes("onion") || nameLower.includes("berry") || nameLower.includes("orange")) {
    matchedCategory = "Produce";
  } else if (nameLower.includes("bread") || nameLower.includes("toast") || nameLower.includes("tortilla") || nameLower.includes("bun") || nameLower.includes("croissant") || nameLower.includes("grain")) {
    matchedCategory = "Bakery";
  } else if (nameLower.includes("frozen") || nameLower.includes("ice cream") || nameLower.includes("pizza")) {
    matchedCategory = "Frozen";
  }
  ...
  shoppingLists[storeName] = list;
  State.user.shoppingLists = shoppingLists;
...
window.printShoppingList = function(storeName) {
  const shoppingLists = State.user.shoppingLists || {};
  const list = shoppingLists[storeName] || [];
  ...
  if (!printWindow) {
    alert("Pop-up blocked! Please allow pop-ups to print the shopping list.");
    return;
  }
  
  const itemsHtml = list.map(item => `
    <li style="font-size: 16px; margin-bottom: 12px; display: flex; align-items: center; list-style-type: none; border-bottom: 1px solid #eee; padding-bottom: 8px;">
      <span style="border: 2px solid #555; width: 20px; height: 20px; display: inline-block; margin-right: 15px; border-radius: 4px; flex-shrink: 0; ${item.checked ? 'background-color: #2eb872; border-color: #2eb872; position: relative;' : ''}">
        ${item.checked ? '<span style="color: white; font-weight: bold; position: absolute; top: -2px; left: 4px; font-size: 14px;">✓</span>' : ''}
      </span>
      <span style="${item.checked ? 'text-decoration: line-through; color: #888;' : ''}">
        <strong>${item.name}</strong> <span style="font-size: 12px; color: #666; font-style: italic;">(${item.category || "Pantry"})</span>
      </span>
    </li>
  `).join('');
  ...
  printWindow.document.write(`
    <html>
      <head>
        <title>Grocerly: ${storeName} Shopping List</title>
        <style>
          body { font-family: 'Helvetica Neue', Arial, sans-serif; padding: 40px; color: #333; max-width: 600px; margin: 0 auto; }
          h1 { border-bottom: 3px solid #2eb872; padding-bottom: 12px; color: #2eb872; font-size: 28px; margin-bottom: 6px; }
          .meta { color: #888; font-size: 13px; margin-bottom: 24px; }
          ul { padding: 0; }
          .print-btn { display: inline-block; padding: 8px 18px; font-size: 14px; font-weight: bold; color: white; background: #2eb872; border: none; border-radius: 4px; cursor: pointer; text-decoration: none; margin-bottom: 20px; }
          @media print {
      ...
      <body>
        ...
        <h1>${storeName} Shopping List</h1>
        <div class="meta">Created on ${new Date().toLocaleDateString()} via Grocerly App</div>
...
window.checkoutShoppingList = function(storeName) {
  const shoppingLists = State.user.shoppingLists || {};
  const list = shoppingLists[storeName] || [];
  const checkedItems = list.filter(item => item.checked);
  
  if (checkedItems.length === 0) {
    alert("Please check off at least one item that you purchased!");
    return;
  }
  
  const confirmCheckout = confirm(`Are you sure you want to complete shopping? This will replenish/add the ${checkedItems.length} checked items into your storage inventory.`);
  ...
  checkedItems.forEach(checkedItem => {
    // Look for exact/similar item in inventory
    const existing = State.inventory.find(i => 
      i.name.toLowerCase().trim() === checkedItem.name.toLowerCase().trim() && 
      (i.store || "").toLowerCase().trim() === storeName.toLowerCase().trim()
    );
    
    if (existing) {
      // Replenish quantity to 100% (or reset remaining percent / qty)
      existing.currentQty = existing.originalQty;
      existing.remainingPercent = 100;
      existing.purchaseDate = new Date().toISOString().split("T")[0];
      
      // Update expiry date based on USDA shelf life or 7 days
      const shelfDays = USDA_SHELF_LIFE[existing.category] ? USDA_SHELF_LIFE[existing.category].fridge : 7;
      const expDate = new Date();
      expDate.setDate(expDate.getDate() + shelfDays);
      existing.expiryDate = expDate.toISOString().split("T")[0];
      existing.isEstimatedDate = true;
      
      replenishedCount++;
    } else {
      // Create new inventory item
      const category = checkedItem.category || "Pantry";
      const shelfDays = USDA_SHELF_LIFE[category] ? USDA_SHELF_LIFE[category].fridge : 7;
      const expDate = new Date();
      expDate.setDate(expDate.getDate() + shelfDays);
      ...
      const newGroc = {
        ...
        currentQty: 1,
        unit: "pcs",
        price: 3.50, // default placeholder price
        store: storeName,
        purchaseDate: new Date().toISOString().split("T")[0],
        expiryDate: expDate.toISOString().split("T")[0],
        isEstimatedDate: true,
        storageLoc: category === "Pantry" ? "Pantry" : "Fridge",
        timesUsed: 0,
        costPerUnit: 3.50,
        notes: "Purchased via digital shopping checklist"
      };
      
      State.inventory.unshift(newGroc);
      addedCount++;
    }
  ...
  alert(`Shopping Complete!\n- ${replenishedCount} existing items replenished back to full quantity.\n- ${addedCount} new items added to your groceries inventory.`);
};

// Bind Checklist Modal Event Listeners
document.addEventListener("DOMContentLoaded", () => {
  // Bind shopping list modal close
  const closeBtn = document.getElementById("btn-shopping-list-modal-close");
  if (closeBtn) {
    closeBtn.onclick = () => {
      document.getElementById("modal-shopping-list").classList.remove("active");
    };
  ...
  // Bind add item input + button
  const addBtn = document.getElementById("btn-shopping-list-add");
  const addInput = document.getElementById("shopping-list-add-name");
  if (addBtn && addInput) {
    const handleAdd = () => {
      const storeName = document.getElementById("modal-shopping-list").dataset.storeName;
      if (storeName && addInput.value.trim()) {
        addShoppingListItem(storeName, addInput.value.trim());
        addInput.value = "";
      }
    };
  ...
  // Bind print button
  const printBtn = document.getElementById("btn-shopping-list-print");
  if (printBtn) {
    printBtn.onclick = () => {
      const storeName = document.getElementById("modal-shopping-list").dataset.storeName;
      if (storeName) {
        printShoppingList(storeName);
      }
    };
  }
  
  // Bind checkout button
  const checkoutBtn = document.getElementById("btn-shopping-list-checkout");
  if (checkoutBtn) {
    checkoutBtn.onclick = () => {
      const storeName = document.getElementById("modal-shopping-list").dataset.storeName;
      if (storeName) {
        checkoutShoppingList(storeName);
      }
    };
  }
});
