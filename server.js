...
const CLIENT_ID = "721951712629-f4i7m3j2vk4bkana2vjbap26res637g3.apps.googleusercontent.com";
...
app.post('/api/auth/google', async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      user = await dbService.createUser(email, "oauth-google-login");
      user.profile.name = name;
      user.profile.avatar = picture;
      await dbService.updateUserProfile(email, user.profile);
    } else {
...
app.put('/api/calendar', async (req, res) => {
  const { email, calendar } = req.body;
  try {
    const saved = await dbService.saveCalendar(email, calendar);
    res.json(saved);
  } catch (error) {
...
app.put('/api/custom-stores', async (req, res) => {
  const { email, customStores } = req.body;
  try {
    const saved = await dbService.saveCustomStores(email, customStores);
    res.json(saved);
  } catch (error) {
