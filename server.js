...
app.get('/api/ai/ideas', async (req, res) => {
  ...
  try {
    ...
    // 1. Determine search terms from groceries, fallback to popular ingredients
    ...
    for (const term of terms) {
      ...
      try {
        ...
        const resData = await fetchJsonFromUrl(url);
        if (resData && resData.meals) {
          apiMeals = apiMeals.concat(resData.meals);
        }
      } catch (err) {
        console.warn(`Failed to fetch recipe ideas from web for term ${term}:`, err.message);
      }
    }
    ...
    // 2. If no matches were fetched from MealDB (e.g. offline or search error), fall back to beautiful template library
    if (matches.length === 0) {
      const templates = [
        {
...
// ----------------------------------------------------
// 7.5. ADMIN CONTROL PANEL ENDPOINTS
// ----------------------------------------------------
...
app.post('/api/feedback', async (req, res) => {
  ...
  try {
    const item = await dbService.submitFeedback(email, name, message);
